...
Feature('1v2 Same Solicitor Claim Journey').tag('@ui-unspec-full-defence');

Scenario('01 Claimant solicitor raises a claim against 2 defendants who have the same solicitor', async ({I}) => {
...
Scenario('02 Claimant solicitor notifies both defendants of claim', async ({I}) => {
  await I.login(config.applicantSolicitorUser);
  await I.notifyClaim();
  // Reinstate the line below when https://tools.hmcts.net/jira/browse/EUI-6286 is fixed
...
Scenario('03 Claimant solicitor notifies defendant solicitor of claim details', async ({I}) => {
  await I.login(config.applicantSolicitorUser);
...
Scenario('07 Defendants solicitor rejects claim for both defendants', async ({I}) => {
  await I.login(config.defendantSolicitorUser);
