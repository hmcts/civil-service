...
http:
  client:
    ...
    maxPerRoute: ${HTTP_MAX_PER_ROUTE:25}
    maxTotal: ${HTTP_MAX_TOTAL:100}
    threshold: ${HTTP_CLIENT_THRESHOLD:5000}
```
...
You can override Feign client settings under `spring.cloud.openfeign.client.config`. The configuration key must match the `name` or `value` attribute of the `@FeignClient` annotation.
    ...
    openfeign:
      client:
        config:
          ...
          ccd-access-data-store-api:
            readTimeout: ${CASE_ACCESS_DATA_STORE_API_READ_TIMEOUT:25000}
          cmc-claim-store:
            readTimeout: ${CMC_CLAIM_STORE_API_READ_TIMEOUT:15000}
```

#### How it works
1. **Timeouts**: When a Feign client is invoked, the `readTimeout` and `connectTimeout` specified in `spring.cloud.openfeign.client.config.<name>` are passed as `Request.Options`. The `InstrumentedFeignClient` passes these to the underlying `ApacheHttpClient`, which overrides the global defaults for that specific request.
