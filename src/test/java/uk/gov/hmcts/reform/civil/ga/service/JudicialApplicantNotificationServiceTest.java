...
class JudicialApplicantNotificationServiceTest {
    ...
    @InjectMocks private JudicialNotificationService judicialNotificationService;

    @Mock private NotificationService notificationService;

    @Mock private SolicitorEmailValidation solicitorEmailValidation;

    @Mock private CaseDetailsConverter caseDetailsConverter;

    @Mock private CoreCaseDataService coreCaseDataService;

    @Mock private JudicialDecisionHelper judicialDecisionHelper;

    @Mock private FeatureToggleService featureToggleService;
    @Mock private GaForLipService gaForLipService;

    @Mock private NotificationsSignatureConfiguration configuration;
    ...
    void setup() {
        ...
        when(notificationsProperties.getJudgeFreeFormOrderRespondentEmailTemplate())
                .thenReturn(SAMPLE_TEMPLATE);
        when(configuration.getHmctsSignature())
                .thenReturn("Online Civil Claims \n HM Courts & Tribunal Service");
        when(configuration.getPhoneContact())
                .thenReturn(
                        "For anything related to hearings, call 0300 123 5577 "
                                + "\n For all other matters, call 0300 123 7050");
        when(configuration.getOpeningHours()).thenReturn("Monday to Friday, 8.30am to 5pm");
        when(configuration.getSpecUnspecContact())
                .thenReturn(
                        "Email for Specified Claims: contactocmc@justice.gov.uk "
                                + "\n Email for Damages Claims: damagesclaims@justice.gov.uk");
        when(configuration.getWelshContact())
                .thenReturn("E-bost: ymholiadaucymraeg@justice.gov.uk");
        when(configuration.getSpecContact()).thenReturn("Email: contactocmc@justice.gov.uk");
        when(configuration.getWelshHmctsSignature())
                .thenReturn(
                        "Hawliadau am Arian yn y Llys Sifil Ar-lein \n"
                                + " Gwasanaeth Llysoedd a Thribiwnlysoedd EF");
        when(configuration.getWelshPhoneContact()).thenReturn("Ffôn: 0300 303 5174");
        when(configuration.getWelshOpeningHours())
                .thenReturn("Dydd Llun i ddydd Iau, 9am – 5pm, dydd Gwener, 9am – 4.30pm");
    }
    ...
    class SendNotificationLip {
        ...
        void sendNotificationApplicantConcurrentWrittenRep() {
            ...
            when(caseDetailsConverter.toGeneralApplicationCaseData(any()))
                    .thenReturn(new GeneralApplicationCaseData().build());
            when(gaForLipService.isLipApp(any())).thenReturn(true);
            judicialNotificationService.sendNotification(
                    caseDataForConcurrentWrittenOption(YES, NO), APPLICANT);
        ...
        void sendNotificationApplicantSequentialWrittenRep() {
            ...
            judicialNotificationService.sendNotification(
                    caseData.copy().applicantBilingualLanguagePreference(YES).build(), APPLICANT);
            verify(notificationService, times(1))
                    .sendMail(
                            DUMMY_EMAIL,
                            "ga-judicial-notification-applicant-template-lip-id",
        ...
        void sendNotificationInWelshApplicantSequentialWrittenRep() {
            when(solicitorEmailValidation.validateSolicitorEmail(any(), any()))
                    .thenReturn(
                            caseDataForSequentialWrittenOption(YES, NO)
                                    .copy()
                                    .applicantBilingualLanguagePreference(YES)
                                    .build());
            GeneralApplicationCaseData claimRespondentResponseLan =
                    new GeneralApplicationCaseData()
                            .claimantBilingualLanguagePreference("WELSH")
                            .applicantBilingualLanguagePreference(YES)
                            .build();
            when(caseDetailsConverter.toGeneralApplicationCaseData(any()))
                    .thenReturn(claimRespondentResponseLan);
            when(gaForLipService.isLipApp(any())).thenReturn(true);
            GeneralApplicationCaseData caseData = caseDataForSequentialWrittenOption(YES, NO);
            GeneralApplicationCaseData updatedCasedata =
                    caseData.copy().applicantBilingualLanguagePreference(YES).build();
        ...
        void notificationUncloakShouldSendForDismissal_ApplicantLIP() {
            ...
            judicialNotificationService.sendNotification(
                    caseDataForJudgeDismissal(NO, NO, NO, YES, NO), APPLICANT);
        ...
        void notificationInWelshShouldSendForDismissal_ApplicantLIP() {
            ...
            when(caseDetailsConverter.toGeneralApplicationCaseData(any()))
                    .thenReturn(claimRespondentResponseLan);
            GeneralApplicationCaseData updatedCaseData =
                    caseDataForJudgeDismissal(NO, NO, NO, YES, NO)
                            .copy()
                            .applicantBilingualLanguagePreference(YES)
                            .build();
            when(solicitorEmailValidation.validateSolicitorEmail(any(), any()))
        ...
        void shouldSendNotification_LipRespondent_UncloakedApplication() {
            ...
            GeneralApplicationCaseData caseData =
                    caseDataForJudicialRequestForInformationOfApplication(
                                    NO, NO, NO, SEND_APP_TO_OTHER_PARTY)
                            .copy()
                            .generalAppType(
                                    new GAApplicationType()
                                            .setTypes(applicationTypeSummeryJudgement()))
                            .build();
        ...
        void shouldSendNotification_LipRespondent_Application() {
            ...
            GeneralApplicationCaseData caseData =
                    caseDataForJudicialRequestForInformationOfApplication(
                                    NO, NO, YES, SEND_APP_TO_OTHER_PARTY)
                            .copy()
                            .generalAppType(
                                    new GAApplicationType()
                                            .setTypes(applicationTypeSummeryJudgement()))
        ...
        void shouldSendNotification_LipRespondent_Application_WhenRequestMoreInfo() {
            ...
            GeneralApplicationCaseData caseData =
                    caseDataForJudicialRequestForInformationOfApplication(
                                    YES, YES, YES, REQUEST_MORE_INFORMATION)
                            .copy()
                            .generalAppConsentOrder(NO)
                            .ccdState(CaseState.APPLICATION_ADD_PAYMENT)
                            .generalAppType(
                                    new GAApplicationType().setTypes(applicationTypeVaryOrder()))
                            .build();
        ...
        @Test
        void
                shouldSendNotification_LipRespondent_Application_WhenRequestMoreInfo_WhenNoLIpInvolved() {
        ...
        private GeneralApplicationCaseData caseDataListForHearing() {
            return new GeneralApplicationCaseData()
                    ...
                    .generalAppParentCaseLink(
                            new GeneralAppParentCaseLink()
                                    .setCaseReference(CASE_REFERENCE.toString()))
                    .generalAppType(
                            new GAApplicationType().setTypes(applicationTypeSummeryJudgement()))
                    .isMultiParty(NO)
                    .build();
        }
        ...
        private GeneralApplicationCaseData caseDataForJudicialRequestForInformationOfApplication(
                YesOrNo isRespondentOrderAgreement,
            ...
            return new GeneralApplicationCaseData()
                    ...
                    .generalAppRespondentAgreement(
                            new GARespondentOrderAgreement()
                    ...
                    .generalAppParentCaseLink(
                            new GeneralAppParentCaseLink()
                                    .setCaseReference(CASE_REFERENCE.toString()))
                    .generalAppType(
                            new GAApplicationType().setTypes(applicationTypeToStayTheClaim()))
                    .generalAppPBADetails(new GeneralApplicationPbaDetails())
                    .build();
        }

        private GeneralApplicationCaseData
                caseDataWithSolicitorDataOnlyForApplicationUncloakedJudgeApproveOrEdit(
                        YesOrNo orderAgreement, YesOrNo isWithNotice, YesOrNo isCloaked) {
            return new GeneralApplicationCaseData()
                    .applicationIsCloaked(isCloaked)
                    .ccdCaseReference(CASE_REFERENCE)
                    .generalAppRespondentSolicitors(respondentSolicitors())
                    ...
                    .generalAppParentCaseLink(
                            new GeneralAppParentCaseLink()
                                    .setCaseReference(CASE_REFERENCE.toString()))
                    .generalAppRespondentAgreement(
                            new GARespondentOrderAgreement().setHasAgreed(orderAgreement))
                    .generalAppInformOtherParty(
                            new GAInformOtherParty().setIsWithNotice(isWithNotice))
                    .generalAppType(
                            new GAApplicationType().setTypes(applicationTypeSummeryJudgement()))
        ...
        private GeneralApplicationCaseData
            ...
            return new GeneralApplicationCaseData()
                    ...
                    .generalAppRespondentSolicitors(
                            isGaLip.equals(YES) ? lipRespondent() : respondentSolicitors())
                    .generalAppApplnSolicitor(new GASolicitorDetailsGAspec().setEmail(DUMMY_EMAIL))
                    .businessProcess(new BusinessProcess().setCamundaEvent(JUDGES_DECISION))
                    .generalAppParentCaseLink(
                            new GeneralAppParentCaseLink()
        ...
        private GeneralApplicationCaseData caseDataForJudgeDismissal(
                YesOrNo orderAgreement,
                YesOrNo isWithNotice,
                YesOrNo isCloaked,
                YesOrNo isLipApplicant,
                YesOrNo isLipRespondent) {
            return new GeneralApplicationCaseData()
                    .ccdCaseReference(CASE_REFERENCE)
                    .generalAppRespondentAgreement(
                            new GARespondentOrderAgreement().setHasAgreed(orderAgreement))
        ...
        private Map<String, String> notificationPropertiesSummeryJudgement() {
            ...
            customProp.put(
                    NotificationDataGA.WELSH_CONTACT, "E-bost: ymholiadaucymraeg@justice.gov.uk");
            customProp.put(
            ...
            customProp.put(
                    NotificationDataGA.SPEC_UNSPEC_CONTACT,
                    "Email for Specified Claims: contactocmc@justice.gov.uk "
                            + "\n Email for Damages Claims: damagesclaims@justice.gov.uk");
            customProp.put(
                    NotificationDataGA.HMCTS_SIGNATURE,
                    "Online Civil Claims \n HM Courts & Tribunal Service");
            customProp.put(NotificationDataGA.OPENING_HOURS, "Monday to Friday, 8.30am to 5pm");
            customProp.put(
                    NotificationDataGA.PHONE_CONTACT,
                    "For anything related to hearings, call 0300 123 5577 "
                            + "\n For all other matters, call 0300 123 7050");
            return customProp;
        }
        ...
        private GeneralApplicationCaseData caseDataForConcurrentWrittenOption(
                YesOrNo isGaApplicantLip, YesOrNo isGaRespondentOneLip) {
            return new GeneralApplicationCaseData()
                    .ccdCaseReference(CASE_REFERENCE)
    ...
    class AboutToSubmitCallback {
        ...
        void shouldNotSentNotification_WhenNotificationCriteria_NotMet() {
            ...
            GeneralApplicationCaseData caseData =
                    caseDataForConcurrentWrittenOption().copy().businessProcess(null).build();
            when(solicitorEmailValidation.validateSolicitorEmail(any(), any()))
                    .thenReturn(caseData);
        ...
        void notificationShouldSendThriceForConcurrentWrittenRepsWhenInvoked() {
            ...
            judicialNotificationService.sendNotification(
                    caseDataForConcurrentWrittenOption(), APPLICANT);
            verify(notificationService, times(1))
                    .sendMail(
                            DUMMY_EMAIL,
                            "general-application-apps-judicial-notification-template-id",
        ...
        void notificationShouldSendThriceForSequentialWrittenRepsWhenInvoked() {
            ...
            judicialNotificationService.sendNotification(
                    caseDataForSequentialWrittenOption(), APPLICANT);
            verify(notificationService, times(1))
                    .sendMail(
                            DUMMY_EMAIL,
                            "general-application-apps-judicial-notification-template-id",
        ...
        void notificationShouldSendForConcurrentWrittenRepsWhenInvoked() {
            ...
            judicialNotificationService.sendNotification(
                    caseDataForConcurrentWrittenRepRespondentNotPresent(), APPLICANT);
            verify(notificationService, times(1))
                    .sendMail(
                            DUMMY_EMAIL,
                            "general-application-apps-judicial-notification-template-id",
        ...
        void notificationShouldSendForDismissal() {
            ...
            judicialNotificationService.sendNotification(
                    caseDataForJudgeDismissal(NO, YES, NO, NO, NO, NO), APPLICANT);
            verify(notificationService, times(1))
                    .sendMail(
                            DUMMY_EMAIL,
                            "general-application-apps-judicial-notification-template-id",
        ...
        void notificationShouldSendForDismissal_when_consentOrder() {
            GeneralApplicationCaseData caseData =
                    caseDataForJudgeDismissal(YES, YES, YES, NO, NO, NO)
                            .copy()
                            .generalAppConsentOrder(NO)
                            .build();
        ...
        void notificationShouldSendForDismissal_when_withoutConsentOrder() {
            GeneralApplicationCaseData caseData =
                    caseDataForJudgeDismissal(NO, NO, YES, NO, NO, NO);
        ...
        void notificationUncloakShouldSendForDismissal() {
            ...
            judicialNotificationService.sendNotification(
                    caseDataForJudgeDismissal(NO, NO, NO, NO, NO, NO), APPLICANT);
            verify(notificationService, times(1))
                    .sendMail(
                            DUMMY_EMAIL,
                            "general-application-apps-judicial-notification-template-id",
        ...
        void notificationShouldSendToApplicant_IfApplicationCloakedWithApplicantSolicitor() {
            ...
            judicialNotificationService.sendNotification(
                    caseDataForCloakedApplicationWithSolicitorDataOnly(), APPLICANT);
            verify(notificationService)
                    .sendMail(
                            DUMMY_EMAIL,
                            "general-application-apps-judicial-notification-template-id",
        ...
        @Test
        void
                notificationShouldSendSendToApplicant_and_respondent_IfApplicationApproveOrEditForConsentOrder() {
            GeneralApplicationCaseData caseData =
                    caseDataWithSolicitorDataOnlyForApplicationUncloakedJudgeApproveOrEdit(
                                    YES, YES, NO, NO, NO, NO)
                            .copy()
        ...
        @Test
        void
                notificationShouldSendSendTo_applicantAndRespondent_ifApplicationApproveOrEditFor_withoutConsentOrder() {
            GeneralApplicationCaseData caseData =
                    caseDataWithSolicitorDataOnlyForApplicationUncloakedJudgeApproveOrEdit(
                            NO, NO, YES, NO, NO, NO);
        ...
        void notificationShouldSendIfApplicationIsToAmendStatementOfCase() {
            ...
            judicialNotificationService.sendNotification(
                    caseDataForAmendStatementOfClaim(), APPLICANT);
            verify(notificationService, times(1))
                    .sendMail(
                            DUMMY_EMAIL,
                            "general-application-apps-judicial-notification-template-id",
        ...
        void notificationShouldSendIfJudicialApproval() {
            ...
            judicialNotificationService.sendNotification(
                    caseDataForJudicialApprovalOfApplication(NO, YES), APPLICANT);
            verify(notificationService, times(1))
                    .sendMail(
                            DUMMY_EMAIL,
                            "general-application-apps-judicial-notification-template-id",
        ...
        void notificationShouldSendIfJudicialDirectionOrder() {
            ...
            judicialNotificationService.sendNotification(
                    caseDataForJudicialApprovalOfApplication(NO, NO), APPLICANT);
            verify(notificationService, times(1))
                    .sendMail(
                            DUMMY_EMAIL,
                            "general-application-apps-judicial-notification-template-id",
        ...
        void notificationUncloakShouldSendIfJudicialDirectionOrder() {
            ...
            judicialNotificationService.sendNotification(
                    caseDataForJudicialDirectionOrderOfApplication(YES, NO), APPLICANT);
            verify(notificationService, times(1))
                    .sendMail(
                            DUMMY_EMAIL,
                            "general-application-apps-judicial-notification-template-id",
        ...
        @Test
        void
                notificationShouldSend_ApplicantRespondent_When_JudicialDirectionOrderRep_withConsentOrder() {
        ...
        @Test
        void
                notificationShouldSend_Applicant_When_JudicialDirectionOrderRep_ConsentOrder_withRespondentNull() {
        ...
        void notificationShouldSendIfApplicationUncloakedDismissed_WithoutRespondentEmail() {
            ...
            judicialNotificationService.sendNotification(
                    caseDataForApplicationUncloakedIsDismissed(), APPLICANT);
            verify(notificationService)
                    .sendMail(
                            DUMMY_EMAIL,
                            "general-application-apps-judicial-notification-template-id",
        ...
        void notificationShouldSendIfSequentialWrittenRepsArePresentInList() {
            ...
            judicialNotificationService.sendNotification(
                    caseDataForSequentialWrittenRepInList(), APPLICANT);
            verify(notificationService, times(1))
                    .sendMail(
                            DUMMY_EMAIL,
                            "general-application-apps-judicial-notification-template-id",
        ...
        void notificationShouldSendForApplicationsApprovedWhenRespondentsAreInList() {
            ...
            judicialNotificationService.sendNotification(
                    caseDataForApplicationsApprovedWhenRespondentsAreInList(NO, YES), APPLICANT);
            verify(notificationService, times(1))
                    .sendMail(
                            DUMMY_EMAIL,
                            "general-application-apps-judicial-notification-template-id",
        ...
        @Test
        void
                shouldSendNotificationToRespondent_ForApplicationApprovedUncloaked_WhenRespondentsAreInList() {

            when(solicitorEmailValidation.validateSolicitorEmail(any(), any()))
                    .thenReturn(caseDataForApplicationsApprovedWhenRespondentsAreInList(NO, NO));

            judicialNotificationService.sendNotification(
                    caseDataForApplicationsApprovedWhenRespondentsAreInList(NO, NO), APPLICANT);
            verify(notificationService, times(1))
                    .sendMail(
                            DUMMY_EMAIL,
                            "general-application-apps-judicial-notification-template-id",
        ...
        void notificationShouldSendForApplicationListForHearingWhenRespondentsAreAvailableInList() {
            ...
            judicialNotificationService.sendNotification(
                    caseDataForListForHearingRespondentsAreInList(), APPLICANT);
            verify(notificationService, times(1))
                    .sendMail(
                            DUMMY_EMAIL,
                            "general-application-apps-judicial-notification-template-id",
        ...
        @Test
        void
                notificationShouldSendWhenApplicationIsDismissedByJudgeWhenRespondentsAreAvailableInList() {
        ...
        @Test
        void
                notificationShouldSend_ApplicantRespondent_When_ApplicationIsDismissed_withConsentOrder() {

            GeneralApplicationCaseData caseData =
                    caseDataForCaseDismissedByJudgeRespondentsAreInList(YES, YES, YES)
                            .copy()
                            .generalAppConsentOrder(YES)
                            .build();
        ...
        @Test
        void
                notificationShouldSend_ApplicantAndRespondent_When_ApplicationIsDismissed_withOutConsentOrder() {

            GeneralApplicationCaseData caseData =
                    caseDataForCaseDismissedByJudgeRespondentsAreInList(NO, NO, YES).copy().build();
        ...
        @Test
        void
                shouldSendNotification_WhenAdditionalPaymentReceived_JudgeDismissedApplicationUncloaked() {

            when(solicitorEmailValidation.validateSolicitorEmail(any(), any()))
                    .thenReturn(
                            caseDataForCaseDismissedByJudgeRespondentsAreInList(NO, NO, YES)
                                    .copy()
                                    .generalAppPBADetails(
                                            new GeneralApplicationPbaDetails()
                                                    .setAdditionalPaymentDetails(
        ...
        void notificationShouldSendWhenJudgeApprovesOrderApplicationIsCloak() {
            ...
            judicialNotificationService.sendNotification(
                    caseDataForJudgeApprovedOrderCloakWhenRespondentsArePresentInList(), APPLICANT);
            verify(notificationService)
                    .sendMail(
                            DUMMY_EMAIL,
                            "general-application-apps-judicial-notification-template-id",
        ...
        private GeneralApplicationCaseData caseDataForSequentialWrittenOption() {
            return new GeneralApplicationCaseData()
                    ...
                    .generalAppType(
                            new GAApplicationType()
                                    .setTypes(applicationTypeToGetReliefFromSanctions()))
                    .isMultiParty(NO)
                    .build();
        }
        ...
        private GeneralApplicationCaseData caseDataForJudgeDismissal(
            ...
            return new GeneralApplicationCaseData()
                    ...
                    .defendant1PartyName("DEF")
                    .generalAppRespondentSolicitors(
                            isLipApp.equals(YES) ? lipRespondent() : respondentSolicitors())
                    .judicialDecision(
                            new GAJudicialDecision()
                                    .setDecision(GAJudgeDecisionOption.MAKE_AN_ORDER))
                    .judicialDecisionMakeOrder(
            ...
            return new GeneralApplicationCaseData()
                    ...
                    .generalAppParentCaseLink(
                            new GeneralAppParentCaseLink()
                                    .setCaseReference(CASE_REFERENCE.toString()))
                    .generalAppType(
                            new GAApplicationType().setTypes(applicationTypeToExtendTheClaim()))
                    .isMultiParty(NO)
                    .build();
        }
        ...
        private GeneralApplicationCaseData caseDataForAmendStatementOfClaim() {
            return new GeneralApplicationCaseData()
                    ...
                    .generalAppType(
                            new GAApplicationType()
                                    .setTypes(applicationTypeToAmendStatmentOfClaim()))
                    .build();
        }

        private GeneralApplicationCaseData caseDataForJudicialApprovalOfApplication(
                YesOrNo orderAgreement, YesOrNo isWithNotice) {
            return new GeneralApplicationCaseData()
                    .ccdCaseReference(CASE_REFERENCE)
                    .isMultiParty(NO)
                    .generalAppRespondentAgreement(
        ...
        private GeneralApplicationCaseData caseDataForJudicialDirectionOrderOfApplication(
                YesOrNo orderAgreement, YesOrNo isWithNotice) {
            return new GeneralApplicationCaseData()
                    .ccdCaseReference(CASE_REFERENCE)
                    .generalAppRespondentAgreement(
        ...
        private GeneralApplicationCaseData caseDataForApplicationsApprovedWhenRespondentsAreInList(
                YesOrNo orderAgreement, YesOrNo isWithNotice) {
            return new GeneralApplicationCaseData()
                    .ccdCaseReference(CASE_REFERENCE)
                    .generalAppRespondentSolicitors(respondentSolicitors())
        ...
        private GeneralApplicationCaseData caseDataForCaseDismissedByJudgeRespondentsAreInList(
                YesOrNo orderAgreement, YesOrNo isWithNotice, YesOrNo isCloaked) {
            return new GeneralApplicationCaseData()
                    .ccdCaseReference(CASE_REFERENCE)
                    .generalAppRespondentAgreement(
        ...
        private Map<String, String> notificationPropertiesSummeryJudgement() {
            HashMap<String, String> properties =
                    new HashMap<>(
                            Map.of(
                                    NotificationDataGA.CASE_REFERENCE,
                                    CASE_REFERENCE.toString(),
                                    NotificationDataGA.GENAPP_REFERENCE,
                                    CASE_REFERENCE.toString(),
                                    NotificationDataGA.PARTY_REFERENCE,
                                    PARTY_REFERENCE,
                                    NotificationDataGA.GA_APPLICATION_TYPE,
                                    GeneralApplicationTypes.SUMMARY_JUDGEMENT.getDisplayedValue()));
            properties.put(
        ...
        private List<GeneralApplicationTypes> applicationTypeToStrikeOut() {
            return List.of(GeneralApplicationTypes.STRIKE_OUT);
        }
        ...
        private List<GeneralApplicationTypes> applicationTypeToExtendTheClaim() {
            return List.of(GeneralApplicationTypes.EXTEND_TIME);
        }
        ...
        private List<GeneralApplicationTypes> applicationTypeToAmendStatmentOfClaim() {
            return List.of(GeneralApplicationTypes.AMEND_A_STMT_OF_CASE);
        }
        ...
        private List<GeneralApplicationTypes> applicationTypeToGetReliefFromSanctions() {
            return List.of(GeneralApplicationTypes.RELIEF_FROM_SANCTIONS);
        }
    }
    ...
    class RequestMoreInformation {
        ...
        void notificationShouldSend_IfWithoutNotice_ApplicationContinuesToBeCloaked() {
            GeneralApplicationCaseData caseData =
                    caseDataForJudicialRequestForInformationOfApplication(
                                    NO, NO, YES, NO, NO, SEND_APP_TO_OTHER_PARTY)
                            .copy()
                            .isMultiParty(NO)
                            .build();
        ...
        void notificationShouldSend_IfWithNotice() {
            GeneralApplicationCaseData caseData =
                    caseDataForJudicialRequestForInformationOfApplication(
                            NO, YES, NO, NO, NO, REQUEST_MORE_INFORMATION);
        ...
        @Test
        void
                notificationShouldSend_ApplicantRespondent_When_RequestForInformation_withConsentOrder() {

            GeneralApplicationCaseData caseData =
                    caseDataForJudicialRequestForInformationOfApplication(
                                    YES, YES, YES, NO, NO, REQUEST_MORE_INFORMATION)
                            .copy()
                            .generalAppConsentOrder(YES)
                            .ccdState(CaseState.APPLICATION_SUBMITTED_AWAITING_JUDICIAL_DECISION)
                            .build();
        ...
        @Test
        void
                notificationShouldSend_ApplicantAndRespondent_When_RequestForInformation_withOutConsentOrder() {

            GeneralApplicationCaseData caseData =
                    caseDataForJudicialRequestForInformationOfApplication(
                            NO, NO, YES, NO, NO, REQUEST_MORE_INFORMATION);
            caseData =
                    caseData.copy()
                            .ccdState(CaseState.APPLICATION_SUBMITTED_AWAITING_JUDICIAL_DECISION)
                            .build();
        ...
        @Test
        void
                shouldSendAdditionalPaymentNotification_UncloakedApplication_BeforeAdditionalPaymentMade() {

            GeneralApplicationCaseData caseData =
                    caseDataForJudicialRequestForInformationOfApplication(
                                    NO, NO, NO, NO, NO, SEND_APP_TO_OTHER_PARTY)
                            .copy()
                            .isMultiParty(NO)
                            .build();
        ...
        @Test
        void
                shouldSendAdditionalPaymentNotification_Lip_UncloakedApplication_BeforeAdditionalPaymentMade() {

            GeneralApplicationCaseData caseData =
                    caseDataForJudicialRequestForInformationOfApplication(
                            NO, NO, NO, YES, NO, SEND_APP_TO_OTHER_PARTY);
            when(solicitorEmailValidation.validateSolicitorEmail(any(), any()))
                    .thenReturn(caseData);
            when(caseDetailsConverter.toGeneralApplicationCaseData(any()))
                    .thenReturn(new GeneralApplicationCaseData().build());
        ...
        @Test
        void
                shouldSendAdditionalPaymentNotification_Lip_UncloakedApplication_BeforeAdditionalPaymentMade_WhenDefendantMakes_Claim() {
            when(gaForLipService.isLipResp(any())).thenReturn(true);
            when(gaForLipService.isGaForLip(any())).thenReturn(true);
            when(gaForLipService.isLipApp(any())).thenReturn(true);
            GeneralApplicationCaseData caseData =
                    caseDataForJudicialRequestForInformationOfApplication(
                                    NO, YES, NO, YES, YES, REQUEST_MORE_INFORMATION)
                            .copy()
                            .ccdState(ORDER_MADE)
                            .businessProcess(
                                    new BusinessProcess()
            ...
            HashMap<String, String> properties =
                    new HashMap<>(
                            Map.of(
                                    ...
                                    "partyReferences",
                                            "Claimant Reference: Not provided - Defendant"
    ...
    void testIsNotificationCriteriaSatisfied() {
        ...
        assertThat(isNotificationCriteriaSatisfied(getCaseData(NO, NO, RESPONDENT_EMAIL)))
                .isFalse();
        assertThat(isNotificationCriteriaSatisfied(getCaseData(NO, NO, null))).isFalse();
        assertThat(isNotificationCriteriaSatisfied(getCaseData(NO, YES, null))).isFalse();
        assertThat(isNotificationCriteriaSatisfied(getCaseData(NO, YES, RESPONDENT_EMAIL)))
                .isTrue();
        assertThat(isNotificationCriteriaSatisfied(getCaseData(YES, NO, RESPONDENT_EMAIL)))
                .isFalse();
        ...
        assertThat(isNotificationCriteriaSatisfied(getCaseData(YES, YES, RESPONDENT_EMAIL)))
                .isFalse();
    }

    private GeneralApplicationCaseData getCaseData(
            YesOrNo isUrgent, YesOrNo informOtherParty, String recipient) {
        return new GeneralApplicationCaseData()
                .generalAppUrgencyRequirement(
                        new GAUrgencyRequirement().setGeneralAppUrgency(isUrgent))
                .generalAppInformOtherParty(
                        new GAInformOtherParty().setIsWithNotice(informOtherParty))
                .generalAppRespondentSolicitors(
                        wrapElements(new GASolicitorDetailsGAspec().setEmail(recipient)))
                .build();
    }
    ...
    private GeneralApplicationCaseData caseDataForJudicialRequestForInformationOfApplication(
        ...
        return new GeneralApplicationCaseData()
                .ccdCaseReference(CASE_REFERENCE)
                .generalAppRespondentSolicitors(
                        isLipRespondent.equals(YES) ? lipRespondent() : respondentSolicitors())
                .applicationIsCloaked(isCloaked)
                .isMultiParty(NO)
                .judicialDecision(
                        new GAJudicialDecision()
                ...
                .judicialDecisionRequestMoreInfo(
                        new GAJudicialRequestMoreInfo()
                                ...
                                .setDeadlineForMoreInfoSubmission(deadline))
                .generalAppRespondentAgreement(
                        new GARespondentOrderAgreement().setHasAgreed(isRespondentOrderAgreement))
                ...
                .businessProcess(new BusinessProcess().setCamundaEvent(JUDGES_DECISION))
                .generalAppParentCaseLink(
                        new GeneralAppParentCaseLink().setCaseReference(CASE_REFERENCE.toString()))
    ...
    private List<GeneralApplicationTypes> applicationTypeToStayTheClaim() {
        return List.of(GeneralApplicationTypes.STAY_THE_CLAIM);
    }

    public List<GeneralApplicationTypes> applicationTypeSummeryJudgement() {
        return List.of(GeneralApplicationTypes.SUMMARY_JUDGEMENT);
    }

    public List<GeneralApplicationTypes> applicationTypeVaryOrder() {
        return List.of(GeneralApplicationTypes.VARY_ORDER);
    }
}
