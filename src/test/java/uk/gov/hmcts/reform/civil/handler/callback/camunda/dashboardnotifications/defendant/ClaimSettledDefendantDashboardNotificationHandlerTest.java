...
import static uk.gov.hmcts.reform.civil.handler.callback.camunda.dashboardnotifications.DashboardScenarios.SCENARIO_AAA6_CLAIMANT_INTENT_INFORMS_PAID_POST_CCJ_DEFENDANT;
...
public class ClaimSettledDefendantDashboardNotificationHandlerTest extends BaseCallbackHandlerTest {
    ...
    class AboutToSubmitCallback {
        ...
        @Test
        void shouldRecordScenario_whenSettledPostCCJUsingLinkInNotification() {
            CaseData caseData = CaseDataBuilder.builder().atStateLipClaimSettled()
                .caseDataLip(CaseDataLiP.builder().applicant1SettleClaim(YesOrNo.YES)
            ...
            verify(dashboardApiClient).recordScenario(
                caseData.getCcdCaseReference().toString(),
                SCENARIO_AAA6_CLAIMANT_INTENT_INFORMS_PAID_POST_CCJ_DEFENDANT.getScenario(),
                "BEARER_TOKEN",
                ScenarioRequestParams.builder().params(scenarioParams).build()
            );
        }
