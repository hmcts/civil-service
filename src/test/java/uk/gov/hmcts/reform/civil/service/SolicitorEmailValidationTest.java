...
public class SolicitorEmailValidationTest {
    ...
    @BeforeEach
    void setUp() {
        org.mockito.Mockito.when(featureToggleService.isGaForLipsEnabled()).thenReturn(true);
    }
    ...
    private GeneralApplicationCaseData getGaCaseData(YesOrNo isMultiParty, YesOrNo parentClaimantIsApplicant,
                                                     YesOrNo isLipApp, YesOrNo isLipResp) {
        ...
        GASolicitorDetailsGAspec respondent2 = GASolicitorDetailsGAspec.builder().id("id")
            .email(DUMMY_EMAIL).forename("Respondent Two").organisationIdentifier("3").build();
