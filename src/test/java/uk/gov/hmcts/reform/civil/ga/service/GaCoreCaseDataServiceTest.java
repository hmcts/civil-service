...
class GaCoreCaseDataServiceTest {
    ...
    @Mock private SystemUpdateUserConfiguration userConfig;
    ...
    @InjectMocks private GaCoreCaseDataService service;
    ...
    class TriggerEvent {
        ...
        private final GeneralApplicationCaseData caseData =
                new GeneralApplicationCaseDataBuilder()
                        ...
                        .caseManagementLocation(
                                new CaseLocationCivil()
                                        .setRegion("1")
                                        .setBaseLocation("12334")
                                        .setSiteName("london")
                                        .setSiteName("London SE1")
                                        .setPostcode("SE1 1AA"))
                        .build();
        ...
        @BeforeEach
        void setUp() {
            when(userService.getUserInfo(USER_AUTH_TOKEN))
        ...
        void triggerUpdateLocationEpimdsIdEvent_WhenApplicant1DQRequestedCourtCalled() {
            ...
            when(locationRefDataService.getCourtLocationsByEpimmsId(anyString(), anyString()))
            ...
            service.triggerUpdateCaseManagementLocation(
                    Long.valueOf(CASE_ID), CaseEvent.valueOf(EVENT_ID), "2", "12345", "yes", "yes");
    ...
    class SearchCases {
        ...
        void shouldReturnCases_WhenSearchingCasesAsSystemUpdateUser() {
            ...
            when(coreCaseDataApi.searchCases(
                            USER_AUTH_TOKEN, SERVICE_AUTH_TOKEN, CASE_TYPE, query.toString()))
            ...
            verify(coreCaseDataApi)
                    .searchCases(USER_AUTH_TOKEN, SERVICE_AUTH_TOKEN, CASE_TYPE, query.toString());
    ...
    class SearchGeneralApplicationWithCaseId {
        ...
        @Test
        void
                shouldReturnMoreThan10GeneralApplications_WhenSearchingGeneralApplicationsAsSystemUpdateUser() {
            final Query query =
                    new Query(
                            matchQuery("data.generalAppParentCaseLink.CaseReference", CASE_ID),
                            List.of(
            ...
            SearchResult searchResult = SearchResult.builder().total(11).cases(cases).build();
            ...
            List<CaseDetails> casesFound =
                    service.searchGeneralApplicationWithCaseId(CASE_ID, USER_AUTH_TOKEN).getCases();
        ...
        @Test
        void
                shouldReturnLessThan10GeneralApplications_WhenSearchingGeneralApplicationsAsSystemUpdateUser() {
            final Query query =
                    new Query(
                            matchQuery("data.generalAppParentCaseLink.CaseReference", CASE_ID),
                            List.of(
            ...
            SearchResult searchResult = SearchResult.builder().total(3).cases(cases).build();
    ...
    class Retry {
        ...
        void shouldRetry_startCaseForCaseworker_WhenTokenExpired() {
            ...
            verify(coreCaseDataApi, times(2))
                    .startForCaseworker(
                            anyString(),
                            anyString(),
                            anyString(),
        ...
        void shouldRetry_searchCases_WhenTokenExpired() {
            when(coreCaseDataApi.searchCases(
                            eq(EXPIRED_USER_AUTH_TOKEN), anyString(), anyString(), anyString()))
                    .thenThrow(new RuntimeException("Exception"));

            Query query = new Query(matchAllQuery(), List.of("reference", "other field"), 0);
            service.searchCases(query);
            verify(coreCaseDataApi, times(2))
                    .searchCases(anyString(), anyString(), anyString(), anyString());
        }
        ...
        void shouldRetry_getCase_WhenTokenExpired() {
            when(coreCaseDataApi.getCase(eq(EXPIRED_USER_AUTH_TOKEN), anyString(), anyString()))
                    .thenThrow(new RuntimeException("Exception"));

            service.getCase(1L);
            verify(coreCaseDataApi, times(2)).getCase(anyString(), anyString(), anyString());
        }
    }
}
