...
public class GeneralApplicationCreationNotificationServiceTest {
    ...
    @InjectMocks private GeneralApplicationCreationNotificationService gaNotificationService;
    ...
    @Spy private GaForLipService gaForLipService;
    ...
    class AboutToSubmitCallback {
        ...
        void notificationShouldSendIfGa_Urgent_WithNoticeAndFreeFee() {
            ...
            verify(notificationService, times(2)).sendMail(any(), any(), any(), any());
        }
        ...
        void notificationShouldSendIfGa_Lip_WithNoticeAndFeePaid_defendantLipIsGaRespondent() {
            ...
            verify(notificationService, times(2))
                    .sendMail(
                            any(),
                            eq("general-application-respondent-template-lip-id"),
        ...
        void notificationSendShouldContainSolicitorEmailReferenceIfAdded() {
            GeneralApplicationCaseData caseData =
                    getCaseData(true)
                            .copy()
                            .emailPartyReference(
                                    "Claimant Reference: ABC limited - Defendant Reference:"
                                        + " Defendant Ltd")
                            .isGaRespondentOneLip(YES)
                            .ccdCaseReference(CASE_REFERENCE)
                            .generalAppPBADetails(
            ...
            assertThat(argumentCaptor.getValue().get("partyReferences"))
                    .isEqualTo(
                            "Claimant Reference: ABC limited - Defendant Reference: Defendant Ltd");
        }
        ...
        void notificationRespondentInWelshShouldSendIfGa_Lip_WithNoticeAndFeePaid() {
            ...
            verify(notificationService, times(2))
                    .sendMail(
                            any(),
                            eq("general-application-respondent-welsh-template-lip-id"),
                            argumentCaptor.capture(),
                            any());
            assertThat(argumentCaptor.getValue().get("respondentName")).isEqualTo("DEF");
            assertThat(argumentCaptor.getValue().get("ClaimantvDefendant")).isEqualTo("CL v DEF");
        ...
        private Map<String, String> getNotificationDataMap(boolean isLip) {
            HashMap<String, String> properties =
                    new HashMap<>(
                            Map.of(
                                    NotificationDataGA.APPLICANT_REFERENCE,
                                    "claimant",
                                    ...
                                    NOTIFICATION_DEADLINE.format(
                                            DateTimeFormatter.ofPattern("dd MMMM yyyy")),
                                    NotificationDataGA.GA_LIP_RESP_NAME,
                                    isLip ? "Lip Resp" : "",
                                    NotificationDataGA.CASE_TITLE,
                                    isLip ? "CL v DEF" : ""));
            properties.put(
                ...
                return new GeneralApplicationCaseDataBuilder()
                        ...
                        .businessProcess(
                                new BusinessProcess()
                                        .setStatus(STARTED)
                                        .setProcessInstanceId(PROCESS_INSTANCE_ID))
                        .gaInformOtherParty(new GAInformOtherParty().setIsWithNotice(NO))
                        .gaUrgencyRequirement(new GAUrgencyRequirement().setGeneralAppUrgency(NO))
                        .gaRespondentOrderAgreement(
                                new GARespondentOrderAgreement().setHasAgreed(NO))
                        .ccdCaseReference(CASE_REFERENCE)
    ...
    private OngoingStubbing<GeneralApplicationCaseData>
            getGeneralApplicationCaseDataOngoingStubbing(GeneralApplicationCaseData caseData) {
        return when(solicitorEmailValidation.validateSolicitorEmail(any(), any()))
                .thenReturn(caseData);
    }
}
