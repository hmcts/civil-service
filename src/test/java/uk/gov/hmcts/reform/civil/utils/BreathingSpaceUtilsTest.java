...
class BreathingSpaceUtilsTest {
    ...
    void cannotEnterWhenAllDefendantsHaveLeft() {
        CaseData caseData = liftedCase(LocalDate.now().minusDays(5), LocalDate.now().minusDays(1), false);
    ...
    void addEnteredBreathingSpaceToHistoryAppendsLabelledItemClearsLiftAndSortsByStartDescending() {
        ...
        assertThat(caseData.getBreathing().getDefendant1Details().getStart()).isEqualTo(firstEnter.getStart());
        assertThat(caseData.getBreathing().getDefendant1Details().getExpectedEnd()).isNotNull();
        assertThat(caseData.getBreathing().getDefendant1Details().getReasonToLift()).isEqualTo("reason");
        assertThat(caseData.getBreathing().getDefendant2Details().getStart()).isEqualTo(secondEnter.getStart());
        assertThat(caseData.getBreathing().getDefendant2Details().getReference()).isEqualTo("ref");
        assertThat(caseData.getBreathing().getDefendant2Details().getExpectedEnd()).isNull();
        assertThat(caseData.getBreathing().getDefendant2Details().getReasonToLift()).isNull();
    }
    ...
    void addLiftDetailsToCurrentBreathingSpaceWritesLiftOntoOpenStoredItem() {
        ...
        assertThat(caseData.getBreathing().getDefendant1Details().getReference()).isEqualTo("ref");
        assertThat(caseData.getBreathing().getDefendant1Details().getStart()).isEqualTo(enterInfo.getStart());
        assertThat(caseData.getBreathing().getDefendant1Details().getType()).isEqualTo(BreathingSpaceType.STANDARD);
        assertThat(caseData.getBreathing().getDefendant1Details().getExpectedEnd())
            .isEqualTo(caseData.getBreathing().getLift().getExpectedEnd());
        assertThat(caseData.getBreathing().getDefendant1Details().getReasonToLift()).isEqualTo("reason");
        assertThat(caseData.getBreathing().getDefendant2Details()).isNull();
    }
    ...
    private static CaseData liftedCase(LocalDate start, LocalDate expectedEnd, boolean secondDefendant) {
        CaseData caseData = caseWith(enter(start), null);
        if (secondDefendant) {
            caseData.setAddRespondent2(YesOrNo.YES);
        }
        BreathingSpaceUtils.addEnteredBreathingSpaceToHistory(caseData);
        caseData.getBreathing().setLift(new BreathingSpaceLiftInfo()
