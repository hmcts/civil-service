...
class ClaimSettledClaimantDashboardServiceTest {
    ...
    void shouldRecordScenarioForLipClaimant() {
        givenDashboardParamsCanBeMapped();
    ...
    @Test
    void shouldReturnJudgmentRequestedCancelledScenarioWhenJudgmentBufferEnabledAndJoRequested() {
        when(featureToggleService.isJudgmentBufferEnabled()).thenReturn(true);
        CaseData caseData = CaseDataBuilder.builder().build().toBuilder()
            .isJoRequested(YesOrNo.YES)
            .build();

        assertThat(service.getScenarios(caseData)).containsOnly(
            Map.entry(SCENARIO_AAA6_CASE_SETTLED_JR_CANCELLED_CLAIMANT.getScenario(), true)
        );
    }

    private void givenDashboardParamsCanBeMapped() {
