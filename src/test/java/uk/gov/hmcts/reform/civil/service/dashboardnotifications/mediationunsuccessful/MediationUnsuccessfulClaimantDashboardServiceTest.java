...
class MediationUnsuccessfulClaimantDashboardServiceTest {
    ...
    @InjectMocks
    private MediationUnsuccessfulClaimantDashboardService service;
    ...
    void shouldRecordDefendantNotContactableScenarioWhenCarmEnabledAndDefendantNotContactable() {
        ...
        verify(dashboardScenariosService).recordScenarios(
            AUTH_TOKEN,
            SCENARIO_AAA6_CLAIMANT_MEDIATION_WHEN_DEFENDANT_NOT_CONTACTABLE.getScenario(),
            "1234",
            new ScenarioRequestParams(new HashMap<>())
        );
    }
    ...
    @Test
    void shouldRecordNonAttendanceScenarioWhenCarmEnabledAndBothClaimantAndDefendantNotContactable() {
        CaseData caseData = CaseDataBuilder.builder().build();
        caseData.setApplicant1Represented(YesOrNo.NO);
        caseData.setCcdCaseReference(1234L);
        Mediation mediation = new Mediation();
        mediation.setMediationUnsuccessfulReasonsMultiSelect(
