...
class DefendantResponseDeadlineCheckSearchServiceTest extends ElasticSearchServiceTest {
    ...
    protected Query buildQuery(int fromValue) {
        String fixedNow = getSchedulerStartTimeFromThreadLocal();
        BoolQueryBuilder query = boolQuery()
            .minimumShouldMatch(1)
            .should(boolQuery()
                        .must(rangeQuery("data.respondent1ResponseDeadline").lt(fixedNow))
    ...
    private String getSchedulerStartTimeFromThreadLocal() {
        try {
            ...
            throw new RuntimeException("Unable to retrieve fixed 'now' time from ThreadLocal", e);
        }
    }
}
