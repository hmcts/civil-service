...
class DefendantResponseWelshClaimantDashboardNotificationHandlerTest extends BaseCallbackHandlerTest {
    ...
    void handleEventsReturnsTheExpectedCallbackEvent() {
        assertThat(handler.handledEvents()).contains(CREATE_CLAIMANT_DASHBOARD_NOTIFICATION_FOR_DEFENDANT_RESPONSE_WELSH);
    }
    ...
    class AboutToSubmitCallback {
        ...
        @Test
        void shouldRecordScenarioWhenDefendantHasWelshLanguagePreferenceAndTaskListShouldBeDisabled_whenInvoked() {
            ...
            CallbackParams params = CallbackParamsBuilder.builder().of(ABOUT_TO_SUBMIT, caseData).request(
                CallbackRequest.builder()
                    .eventId(CREATE_CLAIMANT_DASHBOARD_NOTIFICATION_FOR_DEFENDANT_RESPONSE_WELSH.name()).build()
            ).build();
            ...
            verify(dashboardScenariosService).recordScenarios(
                "BEARER_TOKEN",
                "Scenario.AAA6.Disable.View.Response.Link",
                caseData.getCcdCaseReference().toString(),
                ScenarioRequestParams.builder().params(scenarioParams).build()
            );
        }
        ...
        @Test
        void shouldRecordScenarioWhenClaimantHasWelshLanguagePreferenceAndTaskListShouldBeDisabled_whenInvoked() {
            when(toggleService.isGaForWelshEnabled()).thenReturn(true);
            CaseData caseData = CaseDataBuilder.builder().atStateTrialReadyCheck().build();
            caseData = caseData.toBuilder().applicant1Represented(NO).claimantBilingualLanguagePreference("BOTH")
                .caseDataLiP(CaseDataLiP.builder()
                                 .respondent1LiPResponse(RespondentLiPResponse.builder()
