...
class CaseProceedOfflineDefendantDashboardServiceTest {
    ...
    @Mock private uk.gov.hmcts.reform.civil.service.FeatureToggleService toggleService;
    ...
    void setup() {
        ...
        service =
                new CaseProceedOfflineDefendantDashboardService(
        ...
        void shouldRecordScenario_whenCaseProgressionEnabledAndActiveJudgment() {
            ...
            verify(dashboardScenariosService)
                    .recordScenarios(
                            eq(AUTH_TOKEN),
                            eq(
                                    SCENARIO_AAA6_UPDATE_CASE_PROCEED_IN_CASE_MAN_DEFENDANT
                                            .getScenario()),
                            eq(caseData.getCcdCaseReference().toString()),
                            any());
        }
        ...
        void shouldRecordQueryScenario_whenCitizenQueryAwaitingResponse() {
            ...
            verify(dashboardScenariosService)
                    .recordScenarios(
                            eq(AUTH_TOKEN),
                            eq(
                                    SCENARIO_AAA6_LIP_QM_CASE_OFFLINE_OPEN_QUERIES_DEFENDANT
                                            .getScenario()),
                            eq(caseData.getCcdCaseReference().toString()),
                            any());
        }
    ...
    private void verifyDeleteNotificationsAndTaskListUpdates(CaseData caseData) {
        ...
        verify(taskListService)
                .makeProgressAbleTasksInactiveForCaseIdentifierAndRoleExcludingCategory(
                        caseData.getCcdCaseReference().toString(), "DEFENDANT", "Applications");
    }
}
