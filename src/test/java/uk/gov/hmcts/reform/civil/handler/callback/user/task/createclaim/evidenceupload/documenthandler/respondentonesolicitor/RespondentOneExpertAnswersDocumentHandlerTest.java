...
public class RespondentOneExpertAnswersDocumentHandlerTest {
    ...
    @BeforeEach
    void setUp() {
        handler = new RespondentOneExpertAnswersDocumentHandler(documentTypeBuilder, uploadDocumentRetriever);
    }
    ...
    void shouldCopyExpertAnswersDocumentsToLegalRep2() {
        ...
        assertEquals(1, builder.build().getDocumentAnswersRes2().size());
    }
    ...
    void shouldRenameDocumentsWithDateAndBundleName() {
        ...
        assertEquals("test otherParty documentAnswer.pdf", caseData.getDocumentAnswersRes().get(0).getValue().getExpertDocument().getDocumentFileName());
    }
}