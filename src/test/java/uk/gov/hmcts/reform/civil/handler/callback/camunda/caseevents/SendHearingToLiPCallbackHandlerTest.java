...
class SendHearingToLiPCallbackHandlerTest extends BaseCallbackHandlerTest {
    ...
    // CaseData caseData =
    // CaseDataBuilder.builder().atStateRespondentFullDefence().build().toBuilder()
    ...
    // // then
    // assertThat(response.getErrors()).isNull();
    // verify(sendHearingBulkPrintService).sendHearingToLIP(any(), any(),
    // eq(TASK_ID_DEFENDANT_HMC), eq(true));
    // }
    ...
    // CaseDocument.builder().documentType(HEARING_FORM_WELSH).documentLink(DOCUMENT_LINK).build();
    // CaseData caseData = CaseDataBuilder.builder()
    // .systemGeneratedCaseDocuments(wrapElements(CaseDocument.builder().documentType(HEARING_FORM).build()))
    ...
    // assertThat(response.getErrors()).isNull();
    // verify(sendHearingBulkPrintService).sendHearingToLIP(any(), any(),
    // eq(TASK_ID_CLAIMANT_HMC), eq(true));
    // }
}
