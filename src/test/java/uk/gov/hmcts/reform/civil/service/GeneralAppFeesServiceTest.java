...
class GeneralAppFeesServiceTest {
    ...
    private static final BigDecimal TEST_FEE_AMOUNT_PENCE =
            new BigDecimal(TEST_FEE_AMOUNT_POUNDS.intValue() * 100);
    private static final FeeLookupResponseDto FEE_POUNDS =
            new FeeLookupResponseDto()
    ...
    @Captor private ArgumentCaptor<String> keywordCaptor;

    @Mock private FeesApiClient feesApiClient;

    @Mock private GeneralAppFeesConfiguration feesConfiguration;

    @InjectMocks private GeneralAppFeesService generalAppFeesService;
    ...
    class FeeForJOWithApplicationType {
        ...
        void shouldThrowsExceptionWhenFeeAmountReturnedIsNull() {
            ...
            when(feesConfiguration.getWithNoticeKeyword())
                    .thenReturn(GENERAL_APPLICATION_WITH_NOTICE);
            FeeLookupResponseDto feeLookupResponse =
                    new FeeLookupResponseDto().setCode(TEST_FEE_CODE).setVersion(1);
            ...
            RuntimeException exception =
                    assertThrows(
                            ...
                            () ->
                                    generalAppFeesService.getFeeForJOWithApplicationType(
                                            GeneralApplicationTypes.SET_ASIDE_JUDGEMENT));
        ...
        static Stream<Arguments> joWithApplicationTypedData() {
            return Stream.of(
                    Arguments.of(
                            ...
                            APPLICATION_TO_VARY_OR_SUSPEND,
                            FEE_PENCE),
                    Arguments.of(
                            GeneralApplicationTypes.SET_ASIDE_JUDGEMENT,
                            GENERAL_APPLICATION_WITH_NOTICE,
                            FEE_PENCE),
                    Arguments.of(
                            GeneralApplicationTypes.OTHER, CERTIFICATE_OF_SATISFACTION, FEE_PENCE));
        }

        @ParameterizedTest
        @MethodSource("joWithApplicationTypedData")
        void shouldReturnFeeForCorrectApplicationType(
                GeneralApplicationTypes generalApplicationTypes, String keyword, Fee expectedFee) {
            // Given
            when(feesConfiguration.getChannel()).thenReturn(DEFAULT_CHANNEL);
            when(feesConfiguration.getJurisdiction1()).thenReturn(CIVIL_JURISDICTION);
            when(feesConfiguration.getJurisdiction2()).thenReturn(CIVIL_JURISDICTION);
            ...
            if (Objects.requireNonNull(generalApplicationTypes) == VARY_ORDER) {
                when(feesConfiguration.getAppnToVaryOrSuspend())
                        .thenReturn(APPLICATION_TO_VARY_OR_SUSPEND);
            } else if (generalApplicationTypes == SET_ASIDE_JUDGEMENT) {
                when(feesConfiguration.getWithNoticeKeyword())
                        .thenReturn(GENERAL_APPLICATION_WITH_NOTICE);
            } else if (generalApplicationTypes == OTHER) {
                when(feesConfiguration.getCertificateOfSatisfaction())
                        .thenReturn(CERTIFICATE_OF_SATISFACTION);
            }
            ...
            // When
            Fee feeForVaryOrder =
                    generalAppFeesService.getFeeForJOWithApplicationType(generalApplicationTypes);
            // Then
    ...
    class FeeForGALiP {
        ...
        void default_types_should_pay_ForGALiP(Boolean isWithConsent, Boolean isWithNotice) {
            ...
            if (isWithNotice) {
                ...
                when(feesConfiguration.getConsentedOrWithoutNoticeKeyword())
                        .thenReturn(GENERAL_APP_WITHOUT_NOTICE);
            }
            ...
            for (GeneralApplicationTypes generalApplicationType : allTypes) {
                Fee feeDto =
                        generalAppFeesService.getFeeForGALiP(
                                List.of(generalApplicationType), isWithConsent, isWithNotice, null);
                assertThat(feeDto).isEqualTo(FEE_PENCE);
            }
            // mix
            Fee feeDto =
                    generalAppFeesService.getFeeForGALiP(
                            allTypes, isWithConsent, isWithNotice, null);
            assertThat(feeDto).isEqualTo(FEE_PENCE);
        }
    }
    ...
    void shouldReturnFeeData_whenCertificateOfSatisfactionOrCancelRequested() {
        ...
        when(feesConfiguration.getCertificateOfSatisfaction())
                .thenReturn(CERT_OF_SATISFACTION_OR_CANCEL);

        Fee feeDto =
                generalAppFeesService.getFeeForGALiP(
                        GeneralAppFeesService.CONFIRM_YOU_PAID_CCJ_DEBT, false, false, null);

        assertThat(feeDto).isEqualTo(FEE_PENCE);
        assertThat(keywordCaptor.getValue()).hasToString(CERT_OF_SATISFACTION_OR_CANCEL);
    }
    ...
    class FeeForGA {
        ...
        @ParameterizedTest
        @CsvSource({"GAOnNotice, NO, YES", "GeneralAppWithoutNotice, NO, NO"})
        void default_types_should_pay_ForGA(
                String noticeType, YesOrNo hasAgreed, YesOrNo isWithNotice) {
            when(feesApiClient.lookupFee(
                            anyString(),
                            anyString(),
                            anyString(),
            ...
            for (GeneralApplicationTypes generalApplicationType : allTypes) {
                CaseData caseData =
                        getFeeCase(List.of(generalApplicationType), hasAgreed, isWithNotice, null);
                Fee feeDto = generalAppFeesService.getFeeForGA(caseData);
                assertThat(feeDto).isEqualTo(FEE_PENCE);
            }
            // mix
            CaseData caseData = getFeeCase(allTypes, hasAgreed, isWithNotice, null);
            Fee feeDto = generalAppFeesService.getFeeForGA(caseData);
            assertThat(feeDto).isEqualTo(FEE_PENCE);
            assertThat(keywordCaptor.getValue()).hasToString(noticeType);
        }
        ...
        @ParameterizedTest
        @MethodSource("adjourn_with_hearingScheduledDate_outside_14daysData")
        void adjourn_with_hearingScheduledDate_outside_14days_should_pay_no_fee(
                GeneralApplicationTypes generalApplicationTypes,
                YesOrNo hasAgreed,
            ...
            assertThat(feeForGA).isEqualTo(expectedFee);
        }
        ...
        @ParameterizedTest
        @MethodSource("generateDefaultTypesData")
        void default_types_should_pay(
                GeneralApplicationTypes generalApplicationTypes,
                YesOrNo hasAgreed,
                YesOrNo isWithNotice,
                Integer daysToAdd,
            ...
            CaseData caseDataWithNotice =
                    getFeeCase(List.of(generalApplicationTypes), hasAgreed, isWithNotice, noOfDays);
        ...
        @ParameterizedTest
        @MethodSource("mixDefaultTypesData")
        void mix_default_types_should_be_charged_a_fee(
                GeneralApplicationTypes generalApplicationTypes,
                YesOrNo hasAgreed,
                YesOrNo isWithNotice,
                Integer daysToAdd,
            ...
            if (generalApplicationTypes == GeneralApplicationTypes.SET_ASIDE_JUDGEMENT
                ...
                when(feesConfiguration.getWithNoticeKeyword())
                        .thenReturn(GENERAL_APPLICATION_WITH_NOTICE);
            } else if (generalApplicationTypes
                    == GeneralApplicationTypes.VARY_PAYMENT_TERMS_OF_JUDGMENT) {
                when(feesConfiguration.getAppnToVaryOrSuspend())
                        .thenReturn(APPLICATION_TO_VARY_OR_SUSPEND);
            }
            ...
            CaseData caseDataOutside14Days =
                    getFeeCase(randomList, hasAgreed, isWithNotice, noOfDays);
        ...
        @ParameterizedTest
        @CsvSource({"GAOnNotice, NO, YES", "GeneralAppWithoutNotice, NO, NO"})
        void default_types_should_pay_ForGA_using_general_application_case_data(
                String noticeType, YesOrNo hasAgreed, YesOrNo isWithNotice) {
            when(feesApiClient.lookupFee(
                            anyString(),
                            anyString(),
            ...
            // mix
            GeneralApplicationCaseData caseData =
                    getFeeCaseGeneralApplication(allTypes, hasAgreed, isWithNotice, null);
            Fee feeDto = generalAppFeesService.getFeeForGA(caseData);
            assertThat(feeDto).isEqualTo(FEE_PENCE);
            assertThat(keywordCaptor.getValue()).hasToString(noticeType);
        }
        ...
        @ParameterizedTest
        @MethodSource("adjourn_with_hearingScheduledDate_outside_14daysDataGeneralApplication")
        void
                adjourn_with_hearingScheduledDate_outside_14days_should_pay_no_fee_using_general_application(
                        GeneralApplicationTypes generalApplicationTypes,
                        YesOrNo hasAgreed,
                        YesOrNo isWithNotice,
                        Integer daysToAdd,
        ...
        @ParameterizedTest
        @MethodSource("generateDefaultTypesDataGeneralApplication")
        void default_types_should_pay_general_using_application_case_data(
                GeneralApplicationTypes generalApplicationTypes,
                YesOrNo hasAgreed,
                YesOrNo isWithNotice,
                Integer daysToAdd,
            ...
            LocalDate noOfDays = daysToAdd > 0 ? LocalDate.now().plusDays(daysToAdd) : null;
            GeneralApplicationCaseData caseDataWithNotice =
                    getFeeCaseGeneralApplication(
                            List.of(generalApplicationTypes), hasAgreed, isWithNotice, noOfDays);
            Fee feeDto = generalAppFeesService.getFeeForGA(caseDataWithNotice);
            assertThat(feeDto).isEqualTo(expectedFee);
        }
        ...
        @Test
        void
                shouldReturnFeeData_whenCertificateOfSatisfactionOrCancelRequestedGeneralApplicationCaseData() {
    ...
    private GeneralApplicationCaseData getFeeCaseGeneralApplication(
            List<GeneralApplicationTypes> types,
            YesOrNo hasAgreed,
            YesOrNo isWithNotice,
            LocalDate hearingScheduledDate) {
        ...
        if (Objects.nonNull(hasAgreed)) {
            builder.generalAppRespondentAgreement(
