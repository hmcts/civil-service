...
import uk.gov.hmcts.reform.civil.handler.callback.user.createsdo.fieldsnihl.ScheduleOfLossClaimantDateNihlFieldValidator;
...
class ScheduleOfLossClaimantDateNihlFieldValidatorTest {

    @Mock
    private FieldsNihlUtils fieldsNihlUtils;

    @InjectMocks
    private ScheduleOfLossClaimantDateNihlFieldValidator validator;
    ...
    void shouldAddErrorWhenDateIsInThePast() {
        ...
        when(fieldsNihlUtils.validateFutureDate(caseData.getSdoR2ScheduleOfLoss().getSdoR2ScheduleOfLossClaimantDate()))
    ...
    void shouldNotAddErrorWhenDateIsInTheFuture() {
        CaseData caseData = CaseData.builder()
                .sdoR2ScheduleOfLoss(SdoR2ScheduleOfLoss.builder()
                        .sdoR2ScheduleOfLossClaimantDate(LocalDate.now().plusDays(1))
                        .build())
                .build();
    ...
    void shouldNotAddErrorWhenDateIsNull() {
        CaseData caseData = CaseData.builder()
                .sdoR2ScheduleOfLoss(SdoR2ScheduleOfLoss.builder()
                        .sdoR2ScheduleOfLossClaimantDate(null)
                        .build())
                .build();
