...
class GenerateApplicationDraftCallbackHandlerTest
        extends GeneralApplicationBaseCallbackHandlerTest {

    @Mock private Time time;
    @Mock private GeneralApplicationDraftGenerator generalApplicationDraftGenerator;
    @InjectMocks private GenerateApplicationDraftCallbackHandler handler;
    ...
    void handleEventsReturnsTheExpectedCallbackEvent() {
        assertThat(handler.handledEvents()).contains(GENERATE_DRAFT_DOCUMENT);
    }

    @Test
    void
            shouldGenerateApplicationDraftDocument_whenAboutToSubmitEventIsCalledAndWithoutNotice_LR() {
                ...
                caseData.copy()
                        .generalAppPBADetails(
                                new GeneralApplicationPbaDetails()
                                        ...
                                        .setFee(new Fee().setCode("NotFree")))
                        .build();
        CallbackParams params = callbackParamsOf(caseData, ABOUT_TO_SUBMIT);
        when(generalApplicationDraftGenerator.generate(
                        any(GeneralApplicationCaseData.class), anyString()))
                .thenReturn(PDFBuilder.APPLICATION_DRAFT_DOCUMENT);
        var response = (AboutToStartOrSubmitCallbackResponse) handler.handle(params);

        verify(generalApplicationDraftGenerator)
                .generate(any(GeneralApplicationCaseData.class), eq("BEARER_TOKEN"));

        GeneralApplicationCaseData updatedData =
                mapper.convertValue(response.getData(), GeneralApplicationCaseData.class);
    ...
    @Test
    void
            shouldNotGenerateApplicationDraftDocument_whenAboutToSubmitEventIsCalledAndWithNoticeAndNotUrgent_LR() {
    ...
    @Test
    void
            shouldGenerateApplicationDraftDocument_whenAboutToSubmitEventIsCalledAndWithNoticeAndUrgent_LR() {
    ...
    @Test
    void
            shouldGenerateApplicationDraftDocument_whenAboutToSubmitEventIsCalledAndWithoutNoticeAndNonUrgent_LR() {
        when(gaForLipService.isGaForLip(any())).thenReturn(false);
    ...
    void shouldSetTranslationDocumentsForWlu_Lip() {
        ...
        assertThat(updatedData.getPreTranslationGaDocumentType())
                .isEqualTo(PreTranslationGaDocumentType.APPLICATION_SUMMARY_DOC);
    }
    ...
    void shouldSetTranslationDocumentsForWlu_LipWhenRespondentRespond() {
        ...
        GeneralApplicationCaseData caseData =
                getSampleGeneralApplicationCaseDataWithResponse(YES, YES, YES);
        caseData =
                caseData.copy()
        ...
        assertThat(updatedData.getPreTranslationGaDocuments().getFirst().getValue())
                .isEqualTo(PDFBuilder.APPLICATION_DRAFT_DOCUMENT);
        assertThat(updatedData.getPreTranslationGaDocumentType())
                .isEqualTo(PreTranslationGaDocumentType.RESPOND_TO_APPLICATION_SUMMARY_DOC);
    }
    ...
    void shouldGenerateDraftDocument_FreeFeeCode_ConsentApp_LR() {
        ...
        caseData =
                caseData.copy()
                        ...
                        .build();
        when(generalAppFeesService.isFreeApplication(any(GeneralApplicationCaseData.class)))
                .thenReturn(true);
        when(generalApplicationDraftGenerator.generate(
                        any(GeneralApplicationCaseData.class), anyString()))
                .thenReturn(PDFBuilder.APPLICATION_DRAFT_DOCUMENT);
    ...
    @Test
    void
            shouldGenerateApplicationDraftDocument_whenAboutToSubmitEventIsCalledAndWithNoticeAndUrgent_Lip() {
        GeneralApplicationCaseData caseData = getSampleGeneralApplicationCaseDataLip(YES, YES, YES);
        when(gaForLipService.isGaForLip(any())).thenReturn(true);
        caseData =
                caseData.copy()
    ...
    @Test
    void
            shouldGenerateApplicationDraftDocument_whenAboutToSubmitEventIsCalledAndWithNoticeAndUrgentbefore_Lip() {
        GeneralApplicationCaseData caseData = getSampleGeneralApplicationCaseData(YES, YES, YES);
        when(gaForLipService.isGaForLip(any())).thenReturn(true);
        caseData =
                caseData.copy()
    ...
    void shouldNotGenerateApplicationDraftDocument_whenJudgeHasMadeDecision_LR() {
        ...
        caseData =
                caseData.copy()
                        .judicialDecision(
                                new uk.gov.hmcts.reform.civil.ga.model.genapplication
                                        .GAJudicialDecision())
                        .generalAppPBADetails(
                                new GeneralApplicationPbaDetails()
                                        .setPaymentDetails(
    ...
    void shouldNotGenerateApplicationDraftDocument_whenNotLip_AndNoConditionMet_LR() {
        when(gaForLipService.isGaForLip(any())).thenReturn(false);
        // Non-urgent, With Notice, Paid - but maybe we want to test when NONE of the 4 conditions
        // match.
    ...
    private GeneralApplicationCaseData getSampleGeneralApplicationCaseDataLip(
            YesOrNo isConsented, YesOrNo isTobeNotified, YesOrNo isUrgent) {
    ...
    private GeneralApplicationCaseData getSampleGeneralApplicationCaseDataWithResponse(
            YesOrNo isConsented, YesOrNo isTobeNotified, YesOrNo isUrgent) {
        List<Element<CaseDocument>> gaDraft = new ArrayList<>();
