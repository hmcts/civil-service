...
class ApplicationsProceedOfflineClaimantDashboardServiceTest {
    ...
    @InjectMocks private ApplicationsProceedOfflineClaimantDashboardService service;
    ...
    void shouldOnlyRecordInitiateScenarioWhenNoLiveApplications() {
        List<Element<GeneralApplication>> applications =
                wrapElements(new GeneralApplication().setCaseLink(new CaseLink("123")));
        List<Element<GeneralApplicationsDetails>> claimantDetails =
                wrapElements(new GeneralApplicationsDetails().setCaseState("Application Closed"));
