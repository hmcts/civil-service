...
public class NotifyHearingNoticeDefendantHandlerTest
        extends GeneralApplicationBaseCallbackHandlerTest {

    @InjectMocks private NotifyHearingNoticeDefendantHandler handler;

    @Mock HearingScheduledNotificationService hearingScheduledNotificationService;
    ...
    void shouldSendNotificationToDefendantSuccessfully() {
        var caseData =
                GeneralApplicationCaseDataBuilder.builder()
                        .hearingScheduledApplication(YesOrNo.NO)
                        .build();
        when(hearingScheduledNotificationService.sendNotificationForDefendant(any()))
                .thenReturn(caseData);
        params = callbackParamsOf(caseData, ABOUT_TO_SUBMIT);
    ...
    void shouldSendNotificationToDefendantSuccessfullyWhenWithoutNotice() {
        var caseData =
                GeneralApplicationCaseDataBuilder.builder()
                        .hearingScheduledApplication(YesOrNo.YES)
