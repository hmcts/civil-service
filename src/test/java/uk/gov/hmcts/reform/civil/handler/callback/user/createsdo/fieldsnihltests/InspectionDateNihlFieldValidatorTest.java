...
import uk.gov.hmcts.reform.civil.handler.callback.user.createsdo.fieldsnihl.InspectionDateNihlFieldValidator;
...
class InspectionDateNihlFieldValidatorTest {

    @Mock
    private FieldsNihlUtils fieldsNihlUtils;

    @InjectMocks
    private InspectionDateNihlFieldValidator validator;
    ...
    void shouldAddErrorWhenDateIsInThePast() {
        ...
        when(fieldsNihlUtils.validateFutureDate(caseData.getSdoR2DisclosureOfDocuments().getInspectionDate()))
    ...
    void shouldNotAddErrorWhenDateIsInTheFuture() {
        CaseData caseData = CaseData.builder()
                .sdoR2DisclosureOfDocuments(SdoR2DisclosureOfDocuments.builder()
                        .inspectionDate(LocalDate.now().plusDays(1))
                        .build())
                .build();
