...
class EventEmitterAspectTest {
    ...
    @InjectMocks private EventEmitterAspect aspect;

    @Mock private ProceedingJoinPoint proceedingJoinPoint;

    @Mock private EventEmitterService eventEmitterService;

    @Mock private GaEventEmitterService gaEventEmitterService;
    ...
    @ParameterizedTest
    @EnumSource(
            value = BusinessProcessStatus.class,
            mode = EnumSource.Mode.EXCLUDE,
            names = {"READY"})
    void shouldNotEmitBusinessProcessEvent_whenBusinessProcessStatusIsNotReady(
            BusinessProcessStatus status) throws Throwable {
    ...
    @EnumSource(
            ...
            mode = EnumSource.Mode.EXCLUDE,
            names = {"READY"})
    void shouldNotEmitBusinessProcessCamundaEvent_whenBPStatusIsNotReadyAndPIIdnull(
            BusinessProcessStatus status) {
        final GeneralApplication generalApplication =
                new GeneralApplication()
                        .setBusinessProcess(new BusinessProcess().setStatus(status));
        final CallbackParams callbackParams =
                callbackParamsForSubmittedGeneralApplicationCase(
                        1L, generalApplication, RESPOND_TO_APPLICATION);
    ...
    @EnumSource(
            ...
            mode = EnumSource.Mode.EXCLUDE,
            names = {"READY"})
    void shouldNotEmitBusinessProcessGACamundaEvent_whenBPStatusIsNotReadyAndPIIdnull(
            BusinessProcessStatus status) {
        final GeneralApplicationCaseData caseData =
                new GeneralApplicationCaseData()
                        .businessProcess(new BusinessProcess().setStatus(status))
                        .build();
        final CaseDetails caseDetails = CaseDetailsBuilder.builder().id(1L).data(caseData).build();

        when(caseDetailsConverter.toGeneralApplicationCaseData(eq(caseDetails)))
                .thenReturn(caseData);
    ...
    @Test
    void
            shouldEmitBusinessProcessCamundaEvent_whenCallbackIsSubmittedABPStatusIsReadyAndPIIsNotNull() {
        ...
        final CallbackParams callbackParams =
                callbackParamsForSubmittedGeneralApplicationCase(
                        caseId, generalApplication, RESPOND_TO_APPLICATION);

        aspect.emitBusinessProcessEvent(proceedingJoinPoint, callbackParams);

        verify(gaEventEmitterService)
                .emitBusinessProcessCamundaEvent(caseId, generalApplication, false);
        verify(proceedingJoinPoint).proceed();
    }
    ...
    @Test
    void
            shouldEmitBusinessProcessGACamundaEvent_whenCallbackIsSubmittedABPStatusIsReadyAndPIIsNotNull() {
        final Long caseId = 1L;
        final GeneralApplication generalApplication =
                new GeneralApplication().setBusinessProcess(BusinessProcess.ready(MAKE_DECISION));
        final CallbackParams callbackParams =
                callbackParamsForSubmittedGeneralApplicationCase(
                        caseId, generalApplication, MAKE_DECISION);
    ...
    @Test
    void
            shouldEmitBusinessProcessCamundaEvent_whenCallbackIsSubmittedAndEventIsInitiateGeneralApplication() {
        final Long caseId = 1L;
        final GeneralApplication generalApplication =
                new GeneralApplication()
                        .setBusinessProcess(BusinessProcess.ready(INITIATE_GENERAL_APPLICATION));
        final CallbackParams callbackParams =
                callbackParamsForSubmittedCivilCase(
                        caseId, generalApplication, INITIATE_GENERAL_APPLICATION);
    ...
    @Test
    void
            shouldEmitBusinessProcessCamundaEvent_whenCallbackIsSubmittedAndEventIsInitiateGeneralApplicationCosc() {
        final Long caseId = 1L;
        final GeneralApplication generalApplication =
                new GeneralApplication()
                        .setBusinessProcess(
                                BusinessProcess.ready(INITIATE_GENERAL_APPLICATION_COSC));
        final CallbackParams callbackParams =
                callbackParamsForSubmittedCivilCase(
                        caseId, generalApplication, INITIATE_GENERAL_APPLICATION_COSC);
    ...
    private CallbackParams callbackParamsForSubmittedGeneralApplicationCase(
            Long caseId, GeneralApplication generalApplication, CaseEvent event) {
        return callbackParamsForSubmittedWith(caseId, generalApplication, event, true);
    }

    private CallbackParams callbackParamsForSubmittedCivilCase(
            Long caseId, GeneralApplication generalApplication, CaseEvent event) {
        return callbackParamsForSubmittedWith(caseId, generalApplication, event, false);
    }

    private CallbackParams callbackParamsForSubmittedWith(
            Long caseId, GeneralApplication generalApplication, CaseEvent event, boolean isGaCase) {
        ...
        final CaseDetails caseDetails =
                CaseDetailsBuilder.builder().id(caseId).data(caseData).build();
    ...
    private CallbackParams buildCallbackParams(CallbackType callbackType, CaseData caseData) {
        return CallbackParamsBuilder.builder().of(callbackType, caseData).build();
    }
