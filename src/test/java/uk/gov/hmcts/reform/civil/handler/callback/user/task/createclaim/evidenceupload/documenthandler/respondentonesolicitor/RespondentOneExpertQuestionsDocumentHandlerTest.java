...
public class RespondentOneExpertQuestionsDocumentHandlerTest {
    ...
    @BeforeEach
    void setUp() {
        handler = new RespondentOneExpertQuestionsDocumentHandler(documentTypeBuilder, uploadDocumentRetriever);
    }

    @Test
    void shouldCopyQuestionsDocumentsToLegalRep2() {
        Document document = Document.builder().documentFileName("OriginalQuestions.pdf").build();
        UploadEvidenceExpert uploadEvidenceExpert = UploadEvidenceExpert.builder()
                .expertOptionUploadDate(LocalDate.of(2022, 2, 10))
                .expertOptionName("QuestionsBundle")
                .expertDocument(document)
                .build();

        CaseData caseData = CaseData.builder()
                .documentQuestionsRes(List.of(
        ...
        assertEquals(1, builder.build().getDocumentQuestionsRes2().size());
    }
    ...
    void shouldRenameDocumentsWithDateAndBundleName() {
        ...
        assertEquals("test otherParty documentQuestion.pdf", caseData.getDocumentQuestionsRes().get(0).getValue().getExpertDocument().getDocumentFileName());
    }
}