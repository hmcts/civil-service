...
class ApproveConsentOrderCallbackHandlerTest extends GeneralApplicationBaseCallbackHandlerTest {
    ...
    @Spy private CaseDetailsConverter caseDetailsConverter = new CaseDetailsConverter(mapper);
    ...
    public static final String ORDER_DATE_IN_PAST =
            "The date, by which the order to end" + " should be given, cannot be in past.";

    @InjectMocks private ApproveConsentOrderCallbackHandler handler;

    @Mock private ConsentOrderGenerator consentOrderGenerator;
    ...
    class AboutToStartCallbackHandling {
        ...
        void shouldReturnApproveConsentOrderEndDateEnableWhenApplicationTypeContainsStayTheClaim() {
            ...
            assertThat(data.getGeneralAppDetailsOfOrder())
                    .isEqualTo(data.getApproveConsentOrder().getConsentOrderDescription());
            assertThat(data.getApproveConsentOrder().getShowConsentOrderDate()).isEqualTo(YES);
        }

        @Test
        void
                shouldNotReturnApproveConsentOrderEndDateWhenApplicationTypeDoesNotContainsStayTheClaim() {
            List<GeneralApplicationTypes> types =
                    List.of(
        ...
        void shouldNotAutoCompleteConsentOrderWhenApplicationOrderDetailsEmpty() {
            ...
            CallbackParams params =
                    callbackParamsOf(
                            getGeneralAppCaseDataWithoutOrderDetails(types), ABOUT_TO_START);
            var response = (AboutToStartOrSubmitCallbackResponse) handler.handle(params);
            assertThat(response).isNotNull();
            GeneralApplicationCaseData data =
                    mapper.convertValue(response.getData(), GeneralApplicationCaseData.class);
    ...
    class MidEventToValidate {
        ...
        void shouldGenerateConsentOrderDocument() {
            List<GeneralApplicationTypes> types =
                    List.of(
                            ...
                            (GeneralApplicationTypes.SUMMARY_JUDGEMENT));
            CallbackParams params =
                    callbackParamsOf(getGeneralAppCaseData(types), MID, VALIDATE_CONSENT_ORDER);
            var response = (AboutToStartOrSubmitCallbackResponse) handler.handle(params);
        ...
        void shouldThrowErrorsWhileValidatingDate() {
            ...
            CallbackParams params =
                    callbackParamsOf(
                            getGeneralAppCaseDataForGeneratingDocument(
                                    types, LocalDate.now().minusDays(1), YES),
                            MID,
                            "populate-consent-order-doc");
            var response = (AboutToStartOrSubmitCallbackResponse) handler.handle(params);
        ...
        void shouldNotThrowErrorsWhileValidatingDate() {
            ...
            CallbackParams params =
                    callbackParamsOf(
                            getGeneralAppCaseDataForGeneratingDocument(
                                    types, LocalDate.now().plusDays(1), YES),
                            MID,
                            "populate-consent-order-doc");
            var response = (AboutToStartOrSubmitCallbackResponse) handler.handle(params);
        ...
        void shouldNotThrowErrorsWhileValidatingDateWithOrderDateAsNull() {
            ...
            CallbackParams params =
                    callbackParamsOf(
                            getGeneralAppCaseDataForGeneratingDocument(types, null, YES),
                            MID,
                            "populate-consent-order-doc");
            var response = (AboutToStartOrSubmitCallbackResponse) handler.handle(params);
            assertThat(response.getErrors()).isEmpty();
        ...
        void shouldNotThrowErrorsWhileValidatingDateWithOrderDateAsNullAndDisplayIsNotVisible() {
            ...
            CallbackParams params =
                    callbackParamsOf(
                            getGeneralAppCaseDataForGeneratingDocument(types, null, NO),
                            MID,
                            "populate-consent-order-doc");
            var response = (AboutToStartOrSubmitCallbackResponse) handler.handle(params);
            assertThat(response.getErrors()).isEmpty();
    ...
    class AboutToSubmitHandling {
        ...
        void shouldValidateBusinessProcess() {
            ...
            assertThat(response.getData())
                    ...
                    .isEqualTo("APPROVE_CONSENT_ORDER");
        }
    }
    ...
    public GeneralApplicationCaseData getGeneralAppCaseDataForGeneratingDocument(
            List<GeneralApplicationTypes> types, LocalDate orderDate, YesOrNo showConsent) {
    ...
    public GeneralApplicationCaseData getGeneralAppCaseDataWithoutOrderDetails(
            List<GeneralApplicationTypes> types) {
