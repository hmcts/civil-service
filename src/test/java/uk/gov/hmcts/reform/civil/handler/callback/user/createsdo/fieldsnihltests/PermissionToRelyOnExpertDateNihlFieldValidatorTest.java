...
import uk.gov.hmcts.reform.civil.handler.callback.user.createsdo.fieldsnihl.PermissionToRelyOnExpertDateNihlFieldValidator;
...
class PermissionToRelyOnExpertDateNihlFieldValidatorTest {

    @Mock
    private FieldsNihlUtils fieldsNihlUtils;

    @InjectMocks
    private PermissionToRelyOnExpertDateNihlFieldValidator validator;

    @Test
    void shouldAddErrorWhenDateIsInThePast() {
        CaseData caseData = CaseData.builder()
                .sdoR2PermissionToRelyOnExpert(SdoR2PermissionToRelyOnExpert.builder()
                        .sdoPermissionToRelyOnExpertDate(LocalDate.now().minusDays(1))
                        .build())
                .build();

        List<String> errors = new ArrayList<>();
        when(fieldsNihlUtils.validateFutureDate(caseData.getSdoR2PermissionToRelyOnExpert().getSdoPermissionToRelyOnExpertDate()))
    ...
    void shouldNotAddErrorWhenDateIsInTheFuture() {
        CaseData caseData = CaseData.builder()
                .sdoR2PermissionToRelyOnExpert(SdoR2PermissionToRelyOnExpert.builder()
                        .sdoPermissionToRelyOnExpertDate(LocalDate.now().plusDays(1))
                        .build())
                .build();
    ...
    void shouldNotAddErrorWhenDateIsNull() {
        CaseData caseData = CaseData.builder()
                .sdoR2PermissionToRelyOnExpert(SdoR2PermissionToRelyOnExpert.builder()
                        .sdoPermissionToRelyOnExpertDate(null)
                        .build())
                .build();
