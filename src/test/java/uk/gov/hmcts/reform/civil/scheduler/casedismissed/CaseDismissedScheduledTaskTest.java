...
class CaseDismissedScheduledTaskTest {
    ...
    @InjectMocks
    private CaseDismissedScheduledTask task;
    ...
    @Test
    void shouldPublishDismissClaimEvent() {
        CaseDetails caseDetails = CaseDetails.builder().id(CASE_ID).build();

        task.accept(caseDetails);

        verify(applicationEventPublisher).publishEvent(new DismissClaimEvent(CASE_ID));
    }
