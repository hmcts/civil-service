...
public class DefendantSignSettlementClaimantEmailDTOGeneratorTest {
    ...
    void shouldAddCustomProperties() {
        Party claimantParty = Party.builder().build();
        Party respondentParty = Party.builder().build();
        CaseData caseData = CaseData.builder().applicant1(claimantParty).respondent1(respondentParty).build();
        MockedStatic<PartyUtils> partyUtilsMockedStatic = Mockito.mockStatic(PartyUtils.class);

        String claimantPartyName = "claimant party name";
        partyUtilsMockedStatic.when(() -> getPartyNameBasedOnType(claimantParty, false)).thenReturn(claimantPartyName);
        String defendantPartyName = "defendant party name";
        partyUtilsMockedStatic.when(() -> getPartyNameBasedOnType(respondentParty, false)).thenReturn(defendantPartyName);
        String url = "url";
        ...
        assertThat(updatedProperties).containsEntry(CLAIMANT_NAME, claimantPartyName);
        assertThat(updatedProperties).containsEntry(RESPONDENT_NAME, defendantPartyName);
