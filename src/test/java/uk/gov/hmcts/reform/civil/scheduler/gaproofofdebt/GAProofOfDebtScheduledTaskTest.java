...
class GAProofOfDebtScheduledTaskTest {
    ...
    @InjectMocks
    private GAProofOfDebtScheduledTask task;
    ...
    @Test
    void shouldPublishCoscApplicationProcessorEvent() {
        CaseDetails caseDetails = CaseDetails.builder().id(CASE_ID).build();

        task.accept(caseDetails);

        verify(applicationEventPublisher).publishEvent(new CoscApplicationProcessorEvent(CASE_ID));
    }
