...
class ManageStayWATaskScheduledTaskTest {
    ...
    @InjectMocks
    private ManageStayWATaskScheduledTask task;
    ...
    @Test
    void shouldPublishManageStayWATaskEvent() {
        CaseDetails caseDetails = CaseDetails.builder().id(CASE_ID).build();

        task.accept(caseDetails);

        verify(applicationEventPublisher).publishEvent(new ManageStayWATaskEvent(CASE_ID));
    }
