...
class TrialReadyCheckScheduledTaskTest {
    ...
    @InjectMocks
    private TrialReadyCheckScheduledTask task;
    ...
    @Test
    void shouldPublishTrialReadyCheckEvent() {
        CaseDetails caseDetails = CaseDetails.builder().id(CASE_ID).build();

        task.accept(caseDetails);

        verify(applicationEventPublisher).publishEvent(new TrialReadyCheckEvent(CASE_ID));
    }
