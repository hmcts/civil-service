...
public class CheckUnlessOrderDeadlineEndTaskHandlerTest {
    ...
    @InjectMocks private CheckUnlessOrderDeadlineEndTaskHandler gaUnlessOrderMadeTaskHandler;
    ...
    void init() {
        caseDetailsWithTodayDeadlineNotProcessed =
                getCaseDetails(1L, UNLESS_ORDER, deadLineToday, YesOrNo.NO);
        caseDataWithTodayDeadlineNotProcessed =
                getCaseData(1L, UNLESS_ORDER, deadLineToday, YesOrNo.NO);

        caseDetailsWithTodayDeadlineProcessed =
                getCaseDetails(1L, UNLESS_ORDER, deadLineToday, YesOrNo.YES);
        caseDataWithTodayDeadlineProcessed =
                getCaseData(1L, UNLESS_ORDER, deadLineToday, YesOrNo.YES);

        caseDetailsWithDeadlineCrossedNotProcessed =
                getCaseDetails(3L, UNLESS_ORDER, deadlineCrossed, YesOrNo.NO);
        caseDataWithDeadlineCrossedNotProcessed =
                getCaseData(3L, UNLESS_ORDER, deadlineCrossed, YesOrNo.NO);

        caseDetailsWithDeadlineCrossedProcessed =
                getCaseDetails(4L, UNLESS_ORDER, deadlineCrossed, YesOrNo.YES);
        caseDataWithDeadlineCrossedProcessed =
                getCaseData(4L, UNLESS_ORDER, deadlineCrossed, YesOrNo.YES);

        caseDetailsWithNoDeadline = getCaseDetails(5L, UNLESS_ORDER, null, YesOrNo.NO);
        caseDataWithNoDeadline = getCaseData(5L, UNLESS_ORDER, null, YesOrNo.NO);

        caseDetailsWithFutureDeadline =
                getCaseDetails(6L, UNLESS_ORDER, deadlineInFuture, YesOrNo.NO);
        caseDataWithFutureDeadline = getCaseData(6L, UNLESS_ORDER, deadlineInFuture, YesOrNo.NO);
        caseDetailsWithTodayDeadLineWithOrderProcessedNull =
                getCaseDetails(7L, UNLESS_ORDER, deadLineToday, null);
        caseDataWithTodayDeadLineWithOrderProcessedNull =
                getCaseData(7L, UNLESS_ORDER, deadLineToday, null);
    }
    ...
    void shouldNotSendMessageAndTriggerGaEvent_whenCasesPastDeadlineFoundAndDifferentAppType() {
        when(searchService.getOrderMadeGeneralApplications(ORDER_MADE, UNLESS_ORDER))
                .thenReturn(Set.of(caseDetailsWithDeadlineCrossedProcessed));
    ...
    void shouldNotTriggerBusinessProcessEventWhenIsOrderProcessedIsNull() {
        ...
        verify(coreCaseDataService)
                .triggerGaEvent(
                        ...
                        getCaseData(1L, UNLESS_ORDER, deadLineToday, YesOrNo.YES).toMap(mapper));
        verifyNoMoreInteractions(coreCaseDataService);
        verify(externalTaskService).complete(any(), any());
    }
    ...
    void shouldEmitBusinessProcessEvent_onlyWhen_NotProcessedAndDeadlineReached() {
        ...
        verify(coreCaseDataService)
                .triggerGaEvent(
                        3L,
                        END_SCHEDULER_CHECK_UNLESS_ORDER_DEADLINE,
                        getCaseData(3L, UNLESS_ORDER, deadlineCrossed, YesOrNo.YES).toMap(mapper));
        verifyNoMoreInteractions(coreCaseDataService);
        verify(externalTaskService).complete(any(), any());
    }
    ...
    private GeneralApplicationCaseData getCaseData(
            Long ccdId,
            GeneralApplicationTypes generalApplicationType,
            LocalDate deadline,
            YesOrNo isProcessed) {
                        ...
                        Map.of(
                                "judicialDecisionMakeOrder",
                                        new GAJudicialMakeAnOrder()
                                                .setMakeAnOrder(APPROVE_OR_EDIT)
                                                .setJudgeRecitalText("Sample Text")
                                                .setJudgeApproveEditOptionDateForUnlessOrder(
                                                        deadline)
                                                .setReasonForDecisionText("Sample Test")
                                                .setIsOrderProcessedByUnlessScheduler(isProcessed),
                                "generalAppType",
                                        new GAApplicationType()
                                                .setTypes(List.of(generalApplicationType))))
