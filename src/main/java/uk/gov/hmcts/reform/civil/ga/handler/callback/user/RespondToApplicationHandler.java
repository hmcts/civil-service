...
public class RespondToApplicationHandler extends CallbackHandler
        implements GeneralApplicationCallbackHandler {
    ...
    private static final String CONFIRMATION_SUMMARY =
            "<br/><p> In relation to the following application(s): </p>" + "<ul> %s </ul>" + " %s ";
    ...
    public static final String APPLICATION_RESPONSE_PRESENT =
            "The General Application has already " + "received a response.";
    public static final String RESPONDENT_RESPONSE_EXISTS =
            "The application has already been responded to.";
    ...
    private AboutToStartOrSubmitCallbackResponse applicationValidation(
            CallbackParams callbackParams) {
        GeneralApplicationCaseData caseData = callbackParams.getGeneralApplicationCaseData();
        GeneralApplicationCaseData caseDataBuilder = caseData.copy();
        String authToken = callbackParams.getParams().get(BEARER_TOKEN).toString();

        if (caseData.getGeneralAppType()
                        .getTypes()
                        .contains(GeneralApplicationTypes.VARY_PAYMENT_TERMS_OF_JUDGMENT)
                && caseData.getParentClaimantIsApplicant().equals(NO)) {
            caseDataBuilder.generalAppVaryJudgementType(YesOrNo.YES);
            log.info(
                    "General app vary judgement type for caseId: {}",
                    caseData.getCcdCaseReference());
        } else {
            caseDataBuilder.generalAppVaryJudgementType(NO);
            log.info(
                    "General app does not vary judgement type for caseId: {}",
                    caseData.getCcdCaseReference());
        }
        ...
        caseDataBuilder.hearingDetailsResp(
                new GAHearingDetails()
                        .setHearingPreferredLocation(
                                getLocationsFromList(
                                        locationRefDataService.getCourtLocations(authToken))));
    ...
    private SubmittedCallbackResponse buildResponseConfirmation(CallbackParams callbackParams) {
        ...
        if (gaForLipService.isGaForLip(caseData)
            ...
            if (caseData.getParentClaimantIsApplicant().equals(YesOrNo.NO)
                    && caseData.getGeneralAppType()
                            .getTypes()
                            .contains(GeneralApplicationTypes.VARY_PAYMENT_TERMS_OF_JUDGMENT)) {
                if (gaForLipService.isLipApp(caseData)) {
                    dashboardNotificationService.createOfflineResponseDashboardNotification(
                            caseData, "APPLICANT", authToken);
                }
                if (gaForLipService.isLipResp(caseData)) {
                    dashboardNotificationService.createOfflineResponseDashboardNotification(
                            caseData, "RESPONDENT", authToken);
                }
            } else {
                dashboardNotificationService.createResponseDashboardNotification(
                        caseData, "APPLICANT", authToken);
                dashboardNotificationService.createResponseDashboardNotification(
                        caseData, "RESPONDENT", authToken);
            }
    ...
    public List<String> applicationExistsValidation(CallbackParams callbackParams) {
        GeneralApplicationCaseData caseData = callbackParams.getGeneralApplicationCaseData();
        UserInfo userInfo =
                idamClient.getUserInfo(callbackParams.getParams().get(BEARER_TOKEN).toString());
        ...
        boolean isNotAllowedForLip =
                gaForLipService.isLipResp(caseData)
                        && JudicialDecisionNotificationUtil.isUrgent(caseData);
        List<String> errors = new ArrayList<>();
        if (caseData.getCcdState() == CaseState.APPLICATION_SUBMITTED_AWAITING_JUDICIAL_DECISION
                && !isNotAllowedForLip) {
            errors.add(APPLICATION_RESPONSE_PRESENT);
        }
        if (respondentResponse != null) {
            Optional<Element<GARespondentResponse>> respondentResponseElement =
                    respondentResponse.stream().findAny();
            if (respondentResponseElement.isPresent()) {
                String respondentResponseId =
                        respondentResponseElement.get().getValue().getGaRespondentDetails();
    ...
    private void validateDateRanges(
        ...
        if (YES.equals(isTrialScheduled)) {
            checkTrialScheduled(errors, trialDateFrom, trialDateTo);
        }
    ...
    private void checkTrialScheduled(
            List<String> errors, LocalDate trialDateFrom, LocalDate trialDateTo) {
    ...
    private void checkUnavailable(
            List<String> errors, List<Element<GAUnavailabilityDates>> datesUnavailableList) {
    ...
    private String buildConfirmationSummary(GeneralApplicationCaseData caseData) {
        ...
        String appType =
                genAppTypes.stream()
                        .map(type -> "<li>" + type.getDisplayedValue() + "</li>")
                        .collect(Collectors.joining());
        return format(CONFIRMATION_SUMMARY, appType, JUDGES_REVIEW_MESSAGE);
    }
    ...
    private CallbackResponse submitClaim(CallbackParams callbackParams) {
        ...
        caseDataBuilder.hearingDetailsResp(
                null); // Empty HearingDetails Respondent details as its added in the field
        // RespondetsResponses collection
    ...
    private void addResponseDoc(
        ...
        DocUploadUtils.addDocumentToAddl(
                ...
                documents,
                role,
                CaseEvent.RESPOND_TO_APPLICATION,
                false);
    }

    private GAHearingDetails populateHearingDetailsResp(
            GeneralApplicationCaseData caseData, UserInfo userInfo) {
        GAHearingDetails gaHearingDetailsResp;
        String preferredType =
                caseData.getHearingDetailsResp().getHearingPreferencesPreferredType().name();
        if ((preferredType.equals(PREFERRED_TYPE_IN_PERSON)
                        || caseData.getIsGaRespondentOneLip() == YES)
                ...
                && Objects.nonNull(
                        caseData.getHearingDetailsResp()
                                ...
                                .getValue())) {
            String applicationLocationLabel =
                    caseData.getHearingDetailsResp()
                            .getHearingPreferredLocation()
                            .getValue()
                            .getLabel();
    ...
    private String getRespondentResponsePartyName(
            GeneralApplicationCaseData gaCaseData, UserInfo userInfo) {
        ...
        GeneralApplicationCaseData civilCaseData =
                caseDetailsConverter.toGeneralApplicationCaseData(
                        coreCaseDataService.getCase(
                                Long.parseLong(
                                        gaCaseData
                                                .getGeneralAppParentCaseLink()
                                                .getCaseReference())));

        return checkIfEmailIdMatch(userInfo, civilCaseData, gaCaseData);
    }

    private String checkIfEmailIdMatch(
            UserInfo userInfo,
            GeneralApplicationCaseData civilCaseData,
        ...
        if (!gaCaseData.getParentClaimantIsApplicant().equals(YES)
            ...
            log.info(
                    "Return Civil Claim Defendant two party Name if GA Solicitor Email ID "
        ...
        if (userInfo.getSub().equals(civilCaseData.getRespondentSolicitor1EmailAddress())) {
            ...
            log.info(
                    "Return Civil Claim Defendant One party Name if GA Solicitor Email ID "
    ...
    private List<Element<GARespondentResponse>> addResponse(
            GARespondentResponse gaRespondentResponseBuilder,
            List<Element<GARespondentResponse>> respondentsResponses) {

        List<Element<GARespondentResponse>> newApplication =
                ofNullable(respondentsResponses).orElse(newArrayList());
        newApplication.add(element(gaRespondentResponseBuilder));

        return newApplication;
    }

    private GARespondentResponse buildResponse(
            GeneralApplicationCaseData caseData, UserInfo userInfo) {

        YesOrNo generalOther = NO;
        if (Objects.nonNull(caseData.getGeneralAppConsentOrder())) {
            generalOther = caseData.getGaRespondentConsent();
        } else if (caseData.getGeneralAppType()
                        .getTypes()
                        .contains(GeneralApplicationTypes.VARY_PAYMENT_TERMS_OF_JUDGMENT)
                && caseData.getParentClaimantIsApplicant().equals(NO)
                && ofNullable(caseData.getGaRespondentDebtorOffer()).isPresent()
                && caseData.getGaRespondentDebtorOffer()
                        .getRespondentDebtorOffer()
                        .equals(ACCEPT)) {
            generalOther = YES;
        }
        ...
        gaRespondentResponse
                .setGeneralAppRespondent1Representative(
                        caseData.getGeneralAppRespondent1Representative() == null
                                ? generalOther
    ...
    private String getDebtorReason(GARespondentDebtorOfferGAspec gaRespondentDebtorOffer) {
        if (Objects.nonNull(gaRespondentDebtorOffer)
            ...
            sb.append("Proposed payment plan is ")
                    ...
                    .append(". ");
            if (gaRespondentDebtorOffer
                    .getPaymentPlan()
                    .equals(GADebtorPaymentPlanGAspec.INSTALMENT)) {
                BigDecimal pounds =
                        gaRespondentDebtorOffer
                                .getMonthlyInstalment()
                                .divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_UP);
                sb.append("Proposed instalments per month is ").append(pounds).append(" pounds. ");
            } else {
                sb.append("Proposed set date is ")
                        .append(
                                DateFormatHelper.formatLocalDate(
                                        gaRespondentDebtorOffer.getPaymentSetDate(), "d MMMM yyyy"))
                        .append(". ");
            }
