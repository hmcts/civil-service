...
public class DraftTransitionBuilder extends TransitionBuilder {
    ...
    @Autowired
    public DraftTransitionBuilder(FeatureToggleService featureToggleService, CaseDetailsConverter caseDetailsConverter) {
        super(caseDetailsConverter, featureToggleService, FlowState.Main.DRAFT);
    }

    @Override
    public List<Transition> buildTransitions() {
        return this.moveTo(FlowState.Main.CLAIM_SUBMITTED).onlyWhen(claimSubmittedOneRespondentRepresentative.or(claimSubmitted1v1RespondentOneUnregistered))
            ...
            .moveTo(FlowState.Main.CLAIM_SUBMITTED)
