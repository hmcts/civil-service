    ...
    public StateFlow build(FlowState.Main initialState) {
        return StateFlowBuilder.<FlowState.Main>flow(FLOW_NAME)
            ...
            .state(CLAIM_SUBMITTED)
                .transitionTo(CLAIM_ISSUED_PAYMENT_SUCCESSFUL).onlyIf(not(isLipCase).and(paymentSuccessful))
                .transitionTo(CLAIM_ISSUED_PAYMENT_FAILED).onlyIf(not(isLipCase).and(paymentFailed))
