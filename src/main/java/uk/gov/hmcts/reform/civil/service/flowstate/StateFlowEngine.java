        ...
        return StateFlowBuilder.<FlowState.Main>flow(FLOW_NAME)
            ...
            .state(PENDING_CLAIM_ISSUED_UNREPRESENTED_DEFENDANT)
                .transitionTo(TAKEN_OFFLINE_UNREPRESENTED_DEFENDANT)
                    .onlyIf(takenOfflineBySystem.and(pastAddLegalRepDeadline).and(not(specClaim)))
                .transitionTo(ONE_V_ONE_CASE)
                    .onlyIf(oneVsOneCase.and(specClaim))
                .transitionTo(MULTIPARTY_CASE)
                    .onlyIf(multipartyCase.and(specClaim))
