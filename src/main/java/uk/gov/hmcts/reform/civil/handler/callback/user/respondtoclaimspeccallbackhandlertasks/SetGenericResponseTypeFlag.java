...
public class SetGenericResponseTypeFlag implements CaseTask {
    ...
    public CallbackResponse execute(CallbackParams callbackParams) {
        ...
        handleClaimantResponseType(caseData, updatedData);
        ...
        handleCommonFlags(caseData, updatedData);

        Set<DefendantResponseShowTag> updatedShowConditions = whoDisputesPartAdmission(caseData);
        updateShowConditions(caseData, updatedData, updatedShowConditions);
    ...
    private void handleClaimantResponseType(CaseData caseData, CaseData.CaseDataBuilder<?, ?> updatedData) {
    ...
    private void handleOneVTwoOneLegalRepScenario(CaseData caseData, CaseData.CaseDataBuilder<?, ?> updatedData, MultiPartyScenario multiPartyScenario) {
        if (ONE_V_TWO_ONE_LEGAL_REP.equals(multiPartyScenario)) {
            if (Objects.equals(caseData.getRespondent1ClaimResponseTypeForSpec(), caseData.getRespondent2ClaimResponseTypeForSpec())) {
                ...
                handleDifferentResponse(caseData, updatedData);
            }
        }
    }
    ...
    private void handleDifferentResponse(CaseData caseData, CaseData.CaseDataBuilder<?, ?> updatedData) {
    ...
    private void handleOneVTwoTwoLegalRepScenario(CallbackParams callbackParams, CaseData caseData, CaseData.CaseDataBuilder<?, ?> updatedData,
        ...
        if (ONE_V_TWO_TWO_LEGAL_REP.equals(multiPartyScenario)) {
            setRespondentClaimResponseTypeForSpecGeneric(caseData, updatedData, userInfo);
            setMultiPartyResponseTypeFlags(caseData, updatedData);
            setRespondentClaimResponseTypeForSpecGenericForRespondent2(caseData, updatedData);
        }
    }

    private void setRespondentClaimResponseTypeForSpecGeneric(CaseData caseData, CaseData.CaseDataBuilder<?, ?> updatedData, UserInfo userInfo) {
    ...
    private void setMultiPartyResponseTypeFlags(CaseData caseData, CaseData.CaseDataBuilder<?, ?> updatedData) {
        if ((YES.equals(caseData.getIsRespondent1()) && RespondentResponseTypeSpec.PART_ADMISSION.equals(caseData.getRespondent1ClaimResponseTypeForSpec()))
            || (YES.equals(caseData.getIsRespondent2()) && RespondentResponseTypeSpec.PART_ADMISSION.equals(caseData.getRespondent2ClaimResponseTypeForSpec()))) {
            updatedData.multiPartyResponseTypeFlags(MultiPartyResponseTypeFlags.PART_ADMISSION);
        }
    }

    private void setRespondentClaimResponseTypeForSpecGenericForRespondent2(CaseData caseData, CaseData.CaseDataBuilder<?, ?> updatedData) {
        if (YES.equals(caseData.getIsRespondent2())) {
            updatedData.respondentClaimResponseTypeForSpecGeneric(caseData.getRespondent2ClaimResponseTypeForSpec());
        }
    }

    private void handleCommonFlags(CaseData caseData, CaseData.CaseDataBuilder<?, ?> updatedData) {
        setFullDefenceFlag(caseData, updatedData);
        setSpecFullAdmissionOrPartAdmissionFlag(caseData, updatedData);
        setCounterAdmitOrAdmitPartFlag(caseData, updatedData);
        setSpecFullDefenceOrPartAdmissionFlags(caseData, updatedData);
        setSpecDefenceFullAdmittedRequiredFlag(caseData, updatedData);
        setShowHowToAddTimeLinePageFlag(caseData, updatedData);
        setFullAdmissionAndFullAmountPaidFlag(caseData, updatedData);
        setDefenceAdmitPartPaymentTimeRouteGenericFlag(caseData, updatedData);
        setPartAdmittedByEitherRespondentsFlag(caseData, updatedData);
    }

    private void setFullDefenceFlag(CaseData caseData, CaseData.CaseDataBuilder<?, ?> updatedData) {
    ...
    private void setSpecFullAdmissionOrPartAdmissionFlag(CaseData caseData, CaseData.CaseDataBuilder<?, ?> updatedData) {
    ...
    private void setCounterAdmitOrAdmitPartFlag(CaseData caseData, CaseData.CaseDataBuilder<?, ?> updatedData) {
    ...
    private void setSpecFullDefenceOrPartAdmissionFlags(CaseData caseData, CaseData.CaseDataBuilder<?, ?> updatedData) {
        setSpecFullDefenceOrPartAdmission1V1Flag(caseData, updatedData);
        setSpecFullDefenceOrPartAdmissionFlag(caseData, updatedData);
    }

    private void setSpecFullDefenceOrPartAdmission1V1Flag(CaseData caseData, CaseData.CaseDataBuilder<?, ?> updatedData) {
        if (isRespondent1PartAdmissionOrFullDefence(caseData)) {
            updatedData.specFullDefenceOrPartAdmission1V1(YES);
        }
    }

    private void setSpecFullDefenceOrPartAdmissionFlag(CaseData caseData, CaseData.CaseDataBuilder<?, ?> updatedData) {
        if (isAnyRespondentPartAdmissionOrFullDefence(caseData)) {
    ...
    private boolean isRespondent1PartAdmissionOrFullDefence(CaseData caseData) {
    ...
    private boolean isAnyRespondentPartAdmissionOrFullDefence(CaseData caseData) {
        return isRespondent1PartAdmissionOrFullDefence(caseData)
            || caseData.getRespondent2ClaimResponseTypeForSpec() == RespondentResponseTypeSpec.PART_ADMISSION
            || caseData.getRespondent2ClaimResponseTypeForSpec() == RespondentResponseTypeSpec.FULL_DEFENCE;
    }

    private void setSpecDefenceFullAdmittedRequiredFlag(CaseData caseData, CaseData.CaseDataBuilder<?, ?> updatedData) {
    ...
    private void setShowHowToAddTimeLinePageFlag(CaseData caseData, CaseData.CaseDataBuilder<?, ?> updatedData) {
    ...
    private void setFullAdmissionAndFullAmountPaidFlag(CaseData caseData, CaseData.CaseDataBuilder<?, ?> updatedData) {
    ...
    private void setDefenceAdmitPartPaymentTimeRouteGenericFlag(CaseData caseData, CaseData.CaseDataBuilder<?, ?> updatedData) {
    ...
    private void setPartAdmittedByEitherRespondentsFlag(CaseData caseData, CaseData.CaseDataBuilder<?, ?> updatedData) {
    ...
    private void updateShowConditions(CaseData caseData, CaseData.CaseDataBuilder<?, ?> updatedData, Set<DefendantResponseShowTag> updatedShowConditions) {
        EnumSet<RespondentResponseTypeSpec> anyAdmission = EnumSet.of(RespondentResponseTypeSpec.PART_ADMISSION, RespondentResponseTypeSpec.FULL_ADMISSION);
        addRespondent1AdmitsPartOrFull(caseData, updatedShowConditions, anyAdmission);
        addRespondent2AdmitsPartOrFull(caseData, updatedShowConditions, anyAdmission);
        addSomeoneDisputes(caseData, updatedShowConditions);
        addCurrentAdmitsPartOrFull(caseData, updatedShowConditions, anyAdmission);
        updatedData.showConditionFlags(updatedShowConditions);
    }

    private void addRespondent1AdmitsPartOrFull(CaseData caseData, Set<DefendantResponseShowTag> updatedShowConditions, EnumSet<RespondentResponseTypeSpec> anyAdmission) {
    ...
    private void addRespondent2AdmitsPartOrFull(CaseData caseData, Set<DefendantResponseShowTag> updatedShowConditions, EnumSet<RespondentResponseTypeSpec> anyAdmission) {
    ...
    private void addCurrentAdmitsPartOrFull(CaseData caseData, Set<DefendantResponseShowTag> updatedShowConditions, EnumSet<RespondentResponseTypeSpec> anyAdmission) {
