...
public class GAResponseDeadlineTaskHandler extends BaseExternalTaskHandler {
    ...
    protected Set<CaseDetails> getAwaitingResponseCasesThatArePastDueDate() {
        ...
        return awaitingResponseCases.stream()
            .filter(a -> {
                try {
                    return caseDetailsConverter.toCaseDataGA(a).getGeneralAppNotificationDeadlineDate() != null
                        && now().isAfter(
                        caseDetailsConverter.toCaseDataGA(a).getGeneralAppNotificationDeadlineDate());
                } catch (Exception e) {
