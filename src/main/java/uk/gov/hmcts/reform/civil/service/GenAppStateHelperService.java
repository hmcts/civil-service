...
public class GenAppStateHelperService {
    ...
    public CaseData updateApplicationLocationDetailsInClaim(CaseData caseData) {
        ...
        if (!Collections.isEmpty(caseData.getGeneralApplications())) {
            Pair<CaseLocationCivil, Boolean> caseLocation = genAppService.getWorkAllocationLocation(caseData, null);
            caseData.getGeneralApplications().forEach(generalApplicationElement -> {
                generalApplicationElement.getValue().getCaseManagementLocation().setBaseLocation(caseLocation.getLeft().getBaseLocation());
                generalApplicationElement.getValue().getCaseManagementLocation().setRegion(caseLocation.getLeft().getRegion());
