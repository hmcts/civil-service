...
public class JudicialTimeEstimateHelper {
    ...
    public String getEstimatedHearingLength(CaseData caseData) {
        ...
        int days = hasLength(listForHearing.getJudicialTimeEstimateDays())
            ? Integer.parseInt(listForHearing.getJudicialTimeEstimateDays()) : 0;
        int hours = hasLength(listForHearing.getJudicialTimeEstimateHours())
            ? Integer.parseInt(listForHearing.getJudicialTimeEstimateHours()) : 0;
        int minutes = hasLength(listForHearing.getJudicialTimeEstimateMinutes())
            ? Integer.parseInt(listForHearing.getJudicialTimeEstimateMinutes()) : 0;
        if (days > 0) {
