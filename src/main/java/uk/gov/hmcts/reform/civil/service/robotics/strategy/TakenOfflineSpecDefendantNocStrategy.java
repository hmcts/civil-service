...
public class TakenOfflineSpecDefendantNocStrategy implements EventHistoryStrategy {
    ...
    public boolean supports(CaseData caseData) {
        return caseData != null && hasSpecDefendantNocState(caseData);
    }
    ...
    public void contribute(EventHistory eventHistory, CaseData caseData, String authToken) {
        ...
        log.info(
                "Building taken offline spec defendant NOC robotics event for caseId {}",
    ...
    private boolean hasSpecDefendantNocState(CaseData caseData) {
        ...
        return stateFlow.getStateHistory().stream()
                ...
                .anyMatch(
                        name ->
                                FlowState.Main.TAKEN_OFFLINE_SPEC_DEFENDANT_NOC.fullName().equals(name)
                                        || FlowState.Main.TAKEN_OFFLINE_SPEC_DEFENDANT_NOC_AFTER_JBA
                                                .fullName()
                                                .equals(name));
    }
}
