...
public class ParentCaseUpdateHelper {
    ...
    private static final String GENERAL_APPLICATIONS_DETAILS_FOR_RESP_SOL =
            "respondentSolGaAppDetails";
    private static final String GENERAL_APPLICATIONS_DETAILS_FOR_RESP_SOL_TWO =
            "respondentSolTwoGaAppDetails";
    private static final String GENERAL_APPLICATIONS_DETAILS_FOR_JUDGE =
            "gaDetailsMasterCollection";
    private static final String GENERAL_APPLICATIONS_DETAILS_FOR_WELSH =
    ...
    protected static List<CaseState> DOCUMENT_STATES =
            Arrays.asList(
    ...
    public void updateParentWithGAState(
            GeneralApplicationCaseData generalAppCaseData, String newState) {
        ...
        StartEventResponse startEventResponse =
                coreCaseDataService.startUpdate(parentCaseId, UPDATE_CASE_WITH_GA_STATE);
        GeneralApplicationCaseData caseData =
                caseDetailsConverter.toGeneralApplicationCaseData(
        ...
        if (!isEmpty(respondentSpecficGADetails)) {
            ...
            if (respondentSpecficGADetails.stream()
                    .anyMatch(
                            gaRespondentApp ->
                                    gaRespSolAppFilterCriteria(gaRespondentApp, applicationId))) {
         ...
         *  */
        final List<Element<GeneralApplicationsDetails>> generalApplications =
                updateGaApplicationState(caseData, newState, applicationId, docVisibilityRoles);

        /*
         * Check if the application exists in the Judge List which matches the applicationId
         *  */
        final List<Element<GeneralApplicationsDetails>> gaDetailsMasterCollection =
                updateJudgeGaApplicationState(caseData, newState, applicationId);
        docVisibilityRoles[3] = "Staff";

        List<Element<GeneralApplication>> civilGeneralApplications =
                caseData.getGeneralApplications();
        log.info(
                "updateParentWithGAState() Civil General Applications before for case ID: {}",
                parentCaseId);
        if (generalAppCaseData.getCcdState().equals(PENDING_APPLICATION_ISSUED)
                && !isEmpty(civilGeneralApplications)) {
            List<Element<GeneralApplication>> generalApplicationsList =
                    civilGeneralApplications.stream()
                            .filter(
                                    app ->
                                            app.getValue().getCaseLink() != null
                                                    && !app.getValue()
                                                            .getCaseLink()
                                                            .getCaseReference()
                                                            .equals(applicationId))
                            .toList();
            Optional<Element<GeneralApplication>> newApplicationElement =
                    civilGeneralApplications.stream()
                            .filter(
                                    app ->
                                            app.getValue().getCaseLink() != null
    ...
    protected void updateEvidence(
        ...
        if (generalAppCaseData.getCcdState().equals(PENDING_APPLICATION_ISSUED)) {
            log.info(
                    "PENDING_APPLICATION_ISSUED for Case ID: {}",
            ...
            log.info(
                    "AWAITING_APPLICATION_PAYMENT for Case ID: {}",
                    generalAppCaseData.getCcdCaseReference());
            evidenceRole = docVisibilityRoles;
        }
        if (Objects.nonNull(evidenceRole)) {
            updateSingleTypeByRoles(
                    updateMap, GA_EVIDENCE, evidenceRole, civilCaseData, generalAppCaseData);
        }
    }

    protected void updateSingleTypeByRoles(
            Map<String, Object> updateMap,
            String type,
            String[] roles,
            GeneralApplicationCaseData civilCaseData,
    ...
    protected String findGaCreator(
            GeneralApplicationCaseData civilCaseData,
            GeneralApplicationCaseData generalAppCaseData) {
        log.info(
                "Starting findGaCreator. Evaluating GA creator for Application ID: {}",
                generalAppCaseData.getCcdCaseReference());
        ...
        String creatorId =
                generalAppCaseData.getGeneralAppApplnSolicitor().getOrganisationIdentifier();
        String respondent1OrganisationId =
                civilCaseData.getRespondent1OrganisationPolicy().getOrganisation() != null
                        ? civilCaseData
        ...
        log.debug(
                "GA creator Organisation ID: {}, Respondent 1 Organisation ID: {}",
                creatorId,
                respondent1OrganisationId);
        if (creatorId.equals(respondent1OrganisationId)) {
            log.info("GA creator is Respondent Solicitor 1.");
            return RESPONDENT_SOL_ROLE;
        }
        String respondent2OrganisationId =
                civilCaseData.getRespondent2OrganisationPolicy().getOrganisation() != null
                        ? civilCaseData
        ...
        if (generalAppCaseData.getIsMultiParty().equals(YES)
                ...
                && creatorId.equals(respondent2OrganisationId)) {
    ...
    public void updateMasterCollectionForHwf(GeneralApplicationCaseData generalAppCaseData) {
        ...
        List<Element<GADetailsRespondentSol>> gaDetailsRespondentSol2 =
        ...
        if (generalAppCaseData.getParentClaimantIsApplicant().equals(YES)) {
            Optional<Element<GeneralApplicationsDetails>> claimantCollection =
                    gaClaimantDetails.stream()
                            .filter(
                                    claimantApp ->
                                            applicationFilterCriteria(claimantApp, applicationId))
                            .findAny();

            Optional<Element<GeneralApplicationsDetails>> masterCollection =
                    gaMasterDetails.stream()
                            .filter(
                                    masterCollectionElement ->
                                            applicationFilterCriteria(
                                                    masterCollectionElement, applicationId))
                            .findAny();

            claimantCollection.ifPresent(
                    generalApplicationsDetailsElement -> {
                        if (masterCollection.isEmpty()) {
                            gaMasterDetails.add(
                                    element(
                                            new GeneralApplicationsDetails()
                                                    .setGeneralApplicationType(
                                                            generalApplicationsDetailsElement
                                                                    .getValue()
            ...
            updateJudgeOrClaimantFromRespCollection(
                    generalAppCaseData, applicationId, gaMasterDetails, gaDetailsRespondentSol);
        }

        Map<String, Object> updateMap =
                getUpdatedCaseData(
                        parentCaseData,
                        parentCaseData.getGeneralApplications(),
                        gaClaimantDetails,
    ...
    public void updateJudgeAndRespondentCollectionAfterPayment(
            GeneralApplicationCaseData generalAppCaseData) {
        ...
        if (generalAppCaseData.getParentClaimantIsApplicant().equals(YES)) {
            addClaimantApplicationDetails(
                    generalAppCaseData, applicationId, gaMasterDetails, gaClaimantDetails);
            /*
        ...
        if (generalAppCaseData.getIsMultiParty().equals(YES)
            ...
            updateJudgeOrClaimantFromRespCollection(
                    generalAppCaseData, applicationId, gaMasterDetails, gaDetailsRespondentSol2);
        }
        ...
        */
        if ((generalAppCaseData.getGeneralAppRespondentAgreement().getHasAgreed().equals(NO)
                        && ofNullable(generalAppCaseData.getGeneralAppInformOtherParty())
                                .isPresent()
                        && YES.equals(
                                generalAppCaseData
            ...
            if (generalAppCaseData.getParentClaimantIsApplicant().equals(YES)) {
                ...
                if ((Objects.nonNull(parentCaseData.getRespondent2SameLegalRepresentative())
                                && parentCaseData
                                        .getRespondent2SameLegalRepresentative()
                                        .equals(YES))
                        || generalAppCaseData.getIsMultiParty().equals(NO)) {
            ...
            */
            if (generalAppCaseData.getIsMultiParty().equals(YES)
                ...
                log.info(
                        "Multiparty case and Respondent One initiates the GA for Case ID: {}",
                        generalAppCaseData.getCcdCaseReference());
                updateJudgeOrClaimantFromRespCollection(
                        generalAppCaseData,
                        applicationId,
            ...
            */
            if (generalAppCaseData.getIsMultiParty().equals(YES)
                    && !gaDetailsRespondentSol2.isEmpty()) {
                log.info(
                        "Multiparty case and Respondent Two initiates the GA for Case ID: {}",
                        generalAppCaseData.getCcdCaseReference());
                updateJudgeOrClaimantFromRespCollection(
                        generalAppCaseData,
                        applicationId,
    ...
    public void updateCollectionForWelshApplication(GeneralApplicationCaseData generalAppCaseData) {
        ...
        List<Element<GeneralApplicationsDetails>> gaTranslationDetails =
                ofNullable(parentCaseData.getGaDetailsTranslationCollection())
                        .orElse(newArrayList());
        ...
        if (generalAppCaseData.getParentClaimantIsApplicant().equals(YES)) {
            List<Element<GeneralApplicationsDetails>> gaClaimantDetails =
                    ofNullable(parentCaseData.getClaimantGaAppDetails()).orElse(newArrayList());
            addClaimantApplicationDetails(
                    generalAppCaseData, applicationId, gaTranslationDetails, gaClaimantDetails);
        } else {
            List<Element<GADetailsRespondentSol>> gaDetailsRespondentSol =
                    ofNullable(parentCaseData.getRespondentSolGaAppDetails())
                            .orElse(newArrayList());
            updateJudgeOrClaimantFromRespCollection(
                    generalAppCaseData,
                    applicationId,
        ...
        Map<String, Object> updateMap =
                getUpdateCaseDataForCollection(parentCaseData, gaTranslationDetails);
        CaseDataContent caseDataContent =
                coreCaseDataService.caseDataContentFromStartEventResponse(
                        startEventResponse, updateMap);
    ...
    private void addClaimantApplicationDetails(
            GeneralApplicationCaseData generalAppCaseData,
                                                ...
                                                .setCaseLink(
                                                        new CaseLink(
                                                                String.valueOf(
                                                                        generalAppCaseData
                                                                                .getCcdCaseReference())))
                                                .setParentClaimantIsApplicant(
                                                        generalApplicationsDetailsElement
                                                                .getValue()
                                                                .getParentClaimantIsApplicant()))));
    }

    private void removeApplicationFromTranslationCollection(
            GeneralApplicationCaseData parentCaseData,
            Map<String, Object> updateMap,
            String applicationId) {
        if (featureToggleService.isGaForWelshEnabled()) {
            List<Element<GeneralApplicationsDetails>> gaDetailsTranslationCollection =
                    ofNullable(parentCaseData.getGaDetailsTranslationCollection())
                            .orElse(newArrayList());
            ...
            if (!gaDetailsTranslationCollection.isEmpty()) {
                ...
                var data =
                        gaDetailsTranslationCollection.isEmpty()
                                ? " "
                                : gaDetailsTranslationCollection;
    ...
    private void updateRespCollectionForMultiParty(
            GeneralApplicationCaseData generalAppCaseData,
            String applicationId,
            List<Element<GADetailsRespondentSol>> gaDetailsRespondentSol,
            List<Element<GADetailsRespondentSol>> gaRespondentSol) {
        Optional<Element<GADetailsRespondentSol>> respCollection =
                gaRespondentSol.stream()
                        .filter(
                                respCollectionApp ->
                                        applicationRespFilterCriteria(
                                                respCollectionApp, applicationId))
                        .findAny();

        Optional<Element<GADetailsRespondentSol>> gaToBeAdded =
                gaDetailsRespondentSol.stream()
                        .filter(
                                respCollectionElement ->
                                        gaRespSolAppFilterCriteria(
                                                respCollectionElement, applicationId))
                        .findAny();
    ...
    private void updateRespCollectionFromClaimant(
            GeneralApplicationCaseData generalAppCaseData,
            String applicationId,
            List<Element<GADetailsRespondentSol>> gaDetailsRespondentSol,
            List<Element<GeneralApplicationsDetails>> gaClaimantDetails) {
    ...
    private void updateJudgeOrClaimantFromRespCollection(
            GeneralApplicationCaseData generalAppCaseData,
            String applicationId,
            List<Element<GeneralApplicationsDetails>> gaMasterDetails,
            List<Element<GADetailsRespondentSol>> gaDetailsRespondentSol) {
        if (!gaDetailsRespondentSol.isEmpty()) {
            ...
            Optional<Element<GADetailsRespondentSol>> respondentSolCollection =
                    gaDetailsRespondentSol.stream()
                            .filter(
                                    respondentSolElement2 ->
                                            gaRespSolAppFilterCriteria(
                                                    respondentSolElement2, applicationId))
                            .findAny();
            ...
            */
            if (!masterCollection.isPresent()) {
                log.info(
                        "Application with Case ID {} is added to respondent solicitor collection",
                        generalAppCaseData.getCcdCaseReference());
                respondentSolCollection.ifPresent(
                        respondentSolElement ->
                                gaMasterDetails.add(
                                        element(
                                                new GeneralApplicationsDetails()
                                                        .setGeneralApplicationType(
    ...
    public void updateParentApplicationVisibilityWithNewState(
            GeneralApplicationCaseData generalAppCaseData, String newState) {
         ...
         * */
        Optional<Element<GeneralApplicationsDetails>> generalApplicationsDetails =
                caseData.getGaDetailsMasterCollection().stream()
                        .filter(
                                application ->
                                        applicationFilterCriteria(application, applicationId))
                        .findAny();
        ...
        if (generalApplicationsDetails.isPresent()) {
            ...
            boolean isGaDetailsRespondentSolPresent =
                    gaDetailsRespondentSol.stream()
                            .anyMatch(
                                    gaRespondentApp ->
                                            gaRespSolAppFilterCriteria(
                                                    gaRespondentApp, applicationId));
             ...
             * */
            List<Element<GADetailsRespondentSol>> gaDetailsRespondentSolTwo =
                    ofNullable(caseData.getRespondentSolTwoGaAppDetails()).orElse(newArrayList());

            boolean isGaDetailsRespondentSolTwoPresent =
                    gaDetailsRespondentSolTwo.stream()
                            .anyMatch(
                                    gaRespondentTwoApp ->
                                            gaRespSolAppFilterCriteria(
                                                    gaRespondentTwoApp, applicationId));
            ...
            if (!isGaDetailsRespondentSolTwoPresent) {
                ...
                gaDetailsRespondentSolTwo =
                        updateGaDetailsRespondentTwo(caseData, newState, applicationId);
            }
            ...
            boolean isGaDetailsClaimantPresent =
                    gaDetailsClaimant.stream()
                            .anyMatch(
                                    gaClaimant ->
                                            applicationFilterCriteria(gaClaimant, applicationId));
            ...
            if (!isGaDetailsClaimantPresent) {
                 ...
                 * */
                gaDetailsClaimant =
                        updateGaApplicationState(caseData, newState, applicationId, null);
            }
            ...
            List<Element<GeneralApplicationsDetails>> gaDetailsMasterCollection =
                    updateJudgeGaApplicationState(caseData, newState, applicationId);
            Map<String, Object> updateMap =
                    getUpdatedCaseData(
                            caseData,
    ...
    protected void updateCaseDocument(
            Map<String, Object> updateMap,
            GeneralApplicationCaseData civilCaseData,
            GeneralApplicationCaseData generalAppCaseData,
            String[] roles) {
        for (String role : roles) {
            if (Objects.nonNull(role)) {
                updateCaseDocumentByRole(updateMap, role, civilCaseData, generalAppCaseData);
            }
        }
    }

    protected void updateCaseDocumentByRole(
            Map<String, Object> updateMap,
            String role,
            GeneralApplicationCaseData civilCaseData,
            GeneralApplicationCaseData generalAppCaseData) {
     ...
     * @param type document type. when get from GA data, add 'get' to the name then call getter to
     *     access related GA document field. when get from Civil data, add 'get' with role name then
     *     call getter
     * @param role role name. to be added with type to make the ga getter
     */
    @SuppressWarnings("unchecked")
    protected void updateCaseDocumentByType(
            Map<String, Object> updateMap,
            String type,
            String role,
            GeneralApplicationCaseData civilCaseData,
        ...
        List<Element<?>> civilDocs =
                (List<Element<?>>)
                        ofNullable(civilGetter != null ? civilGetter.invoke(civilCaseData) : null)
                                .orElse(newArrayList());
    ...
    @SuppressWarnings("unchecked")
    protected int checkIfDocumentExists(
            List<Element<?>> civilCaseDocumentList, List<Element<?>> gaCaseDocumentlist) {

        if (gaCaseDocumentlist.get(0).getValue().getClass().equals(CaseDocument.class)) {
            List<Element<CaseDocument>> civilCaseList =
                    civilCaseDocumentList.stream()
                            .map(element -> (Element<CaseDocument>) element)
                            .toList();
            List<Element<CaseDocument>> gaCaseList =
                    gaCaseDocumentlist.stream()
                            .map(element -> (Element<CaseDocument>) element)
                            .toList();

            return civilCaseList.stream()
                    .filter(
                            civilDocument ->
                                    gaCaseList.parallelStream()
                                            .anyMatch(
                                                    gaDocument ->
                                                            gaDocument
                                                                    .getValue()
            ...
            List<Element<Document>> civilCaseList =
                    civilCaseDocumentList.stream()
                            .map(element -> (Element<Document>) element)
                            .toList();

            List<Element<Document>> gaCaseList =
                    gaCaseDocumentlist.stream()
                            .map(element -> (Element<Document>) element)
                            .toList();
    ...
    private List<Element<GeneralApplicationsDetails>> updateGaApplicationState(
            GeneralApplicationCaseData caseData,
            String newState,
            String applicationId,
            String[] roles) {
        log.info(
                "Starting updateGaApplicationState. New state: {}, Application ID: {}",
                newState,
                applicationId);
        List<Element<GeneralApplicationsDetails>> generalApplications =
                ofNullable(caseData.getClaimantGaAppDetails()).orElse(newArrayList());

        if (!isEmpty(generalApplications)
                && generalApplications.stream()
                        .anyMatch(
                                applicant -> applicationFilterCriteria(applicant, applicationId))) {
        ...
        log.info(
                "Completed updateJudgeGaApplicationState for caseId {}",
                caseData.getCcdCaseReference());
        return generalApplications;
    }

    private List<Element<GeneralApplicationsDetails>> updateJudgeGaApplicationState(
            GeneralApplicationCaseData caseData, String newState, String applicationId) {
        log.info(
                "Starting updateJudgeGaApplicationState. New state: {}, Application ID: {}",
                newState,
                applicationId);
        List<Element<GeneralApplicationsDetails>> generalApplications =
                caseData.getGaDetailsMasterCollection();
        if (!isEmpty(generalApplications)
                && generalApplications.stream()
                        .anyMatch(
                                applicant -> applicationFilterCriteria(applicant, applicationId))) {
            log.info(
                    "Application with ID: {} found in the master collection for update.",
                    applicationId);
    ...
    private List<Element<GADetailsRespondentSol>> updateGaDetailsRespondentOne(
            GeneralApplicationCaseData caseData, String newState, String applicationId) {
        List<Element<GADetailsRespondentSol>> gaDetailsRespondentSol =
                ofNullable(caseData.getRespondentSolGaAppDetails()).orElse(newArrayList());
        log.info(
                "Starting update for RespondentSolGaAppDetails with applicationId: {}",
                applicationId);
        if (!isEmpty(gaDetailsRespondentSol)
                && gaDetailsRespondentSol.stream()
                        .anyMatch(
                                respondentOne ->
                                        gaRespSolAppFilterCriteria(respondentOne, applicationId))) {
            log.info(
                    "Matching entry found for applicationId: {}. Updating case state to: {}",
                    applicationId,
                    newState);
            ...
            gaDetailsRespondentSol.stream()
                    ...
                    .setCaseState(newState);
            log.info(
                    "Case state updated to {} for RespondentSol with applicationId: {}",
                    newState,
                    applicationId);
        }
    ...
    private List<Element<GADetailsRespondentSol>> updateGaDetailsRespondentTwo(
            GeneralApplicationCaseData caseData, String newState, String applicationId) {
        List<Element<GADetailsRespondentSol>> gaDetailsRespondentSolTwo =
                ofNullable(caseData.getRespondentSolTwoGaAppDetails()).orElse(newArrayList());
        log.info(
                "Starting update for RespondentSolTwoGaAppDetails with applicationId: {}",
                applicationId);
        ...
        if (!isEmpty(gaDetailsRespondentSolTwo)
                && gaDetailsRespondentSolTwo.stream()
                        .anyMatch(
                                respondentTwo ->
                                        gaRespSolAppFilterCriteria(respondentTwo, applicationId))) {
            log.info(
                    "Matching entry found for applicationId: {}. Updating case state to: {}",
                    applicationId,
                    newState);
            ...
            gaDetailsRespondentSolTwo.stream()
                    ...
                    .setCaseState(newState);
            log.info(
                    "Case state updated to {} for RespondentSolTwo with applicationId: {}",
                    newState,
                    applicationId);
        }
    ...
    private boolean applicationFilterCriteria(
            Element<GeneralApplicationsDetails> gaDetails, String applicationId) {
    ...
    private boolean applicationRespFilterCriteria(
            Element<GADetailsRespondentSol> gaDetails, String applicationId) {
        return gaDetails.getValue() != null
                && gaDetails.getValue().getCaseLink() != null
                && applicationId.equals(gaDetails.getValue().getCaseLink().getCaseReference());
    }

    private boolean gaRespSolAppFilterCriteria(
            Element<GADetailsRespondentSol> gaDetails, String applicationId) {
    ...
    private Map<String, Object> getUpdatedCaseData(
            ...
            List<Element<GADetailsRespondentSol>> respondentSolTwoGaAppDetails,
            List<Element<GeneralApplicationsDetails>> gaDetailsMasterCollection) {
    ...
    private Map<String, Object> getUpdateCaseDataForCollection(
            GeneralApplicationCaseData caseData,
            List<Element<GeneralApplicationsDetails>> gaDetailsTranslationCollection) {
        Map<String, Object> output = caseData.toMap(mapper);
