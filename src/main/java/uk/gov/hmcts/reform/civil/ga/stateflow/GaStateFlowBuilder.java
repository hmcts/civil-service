...
import uk.gov.hmcts.reform.civil.ga.stateflow.StateFlowContext;
import uk.gov.hmcts.reform.civil.ga.stateflow.StateFlowListener;
import uk.gov.hmcts.reform.civil.stateflow.exception.StateFlowException;
import uk.gov.hmcts.reform.civil.ga.stateflow.grammar.Build;
import uk.gov.hmcts.reform.civil.ga.stateflow.grammar.BuildNext;
import uk.gov.hmcts.reform.civil.ga.stateflow.grammar.CreateFlow;
import uk.gov.hmcts.reform.civil.ga.stateflow.grammar.CreateFlowNext;
import uk.gov.hmcts.reform.civil.ga.stateflow.grammar.CreateSubflow;
import uk.gov.hmcts.reform.civil.ga.stateflow.grammar.CreateSubflowNext;
import uk.gov.hmcts.reform.civil.ga.stateflow.grammar.Initial;
import uk.gov.hmcts.reform.civil.ga.stateflow.grammar.InitialNext;
import uk.gov.hmcts.reform.civil.ga.stateflow.grammar.OnlyIf;
import uk.gov.hmcts.reform.civil.ga.stateflow.grammar.OnlyIfNext;
import uk.gov.hmcts.reform.civil.ga.stateflow.grammar.Set;
import uk.gov.hmcts.reform.civil.ga.stateflow.grammar.SetNext;
import uk.gov.hmcts.reform.civil.ga.stateflow.grammar.State;
import uk.gov.hmcts.reform.civil.ga.stateflow.grammar.StateNext;
import uk.gov.hmcts.reform.civil.ga.stateflow.grammar.Subflow;
import uk.gov.hmcts.reform.civil.ga.stateflow.grammar.SubflowNext;
import uk.gov.hmcts.reform.civil.ga.stateflow.grammar.TransitionTo;
import uk.gov.hmcts.reform.civil.ga.stateflow.grammar.TransitionToNext;
import uk.gov.hmcts.reform.civil.ga.stateflow.model.Transition;
...
import static uk.gov.hmcts.reform.civil.ga.stateflow.StateFlowContext.EXTENDED_STATE_CASE_KEY;
import static uk.gov.hmcts.reform.civil.ga.stateflow.StateFlowContext.EXTENDED_STATE_FLAGS_KEY;
 ...
 */
public class GaStateFlowBuilder<S> {
    ...
    private GaStateFlowBuilder(final String flowName) {
        this.flowName = flowName;
        this.stateFlowContext = new StateFlowContext();
    }

    private GaStateFlowBuilder(final String flowName, final StateFlowContext stateFlowContext) {
        this.flowName = flowName;
    ...
    public static <S> CreateFlowNext<S> flow(String flowName) {
        ...
        GaStateFlowBuilder<S> stateFlowBuilder = new GaStateFlowBuilder<>(flowName);
    ...
    public static <S> CreateSubflowNext<S> subflow(String flowName, StateFlowContext stateFlowContext) {
        ...
        GaStateFlowBuilder<S> stateFlowBuilder = new GaStateFlowBuilder<>(flowName, stateFlowContext);
    ...
    protected class Grammar<S>
        ...
        @Override
        public OnlyIfNext<S> onlyIf(Predicate<GeneralApplicationCaseData> condition) {
        ...
        @Override
        public SetNext<S> set(BiConsumer<GeneralApplicationCaseData, Map<String, Boolean>> flags) {
            checkNull(flags, STATE);
            stateFlowContext.getCurrentTransition()
        ...
        public GaStateFlow build() {
                    ...
                    if (transition.getCondition() != null) {
                        transitionConfigurer.guard(
                            context -> transition.getCondition().test(
                                context.getExtendedState().get(EXTENDED_STATE_CASE_KEY, GeneralApplicationCaseData.class)
                            )
                        );
                    }
                    ...
                    if (transition.getDynamicFlags() != null) {
                        transitionConfigurer.action(
                            action -> transition.getDynamicFlags().accept(
                                action.getExtendedState().get(EXTENDED_STATE_CASE_KEY, GeneralApplicationCaseData.class),
                                (Map<String, Boolean>)action.getExtendedState().get(EXTENDED_STATE_FLAGS_KEY, Map.class)
                            )
                        );
                    }
            ...
            return new GaStateFlow(stateMachine);
        }
