...
public class TrialArrangementsNotifyOtherPartyNotificationHandler extends CallbackHandler {
    ...
    protected Map<String, Callback> callbacks() {
        return toggleService.isDashboardServiceEnabled()
            ? Map.of(callbackKey(ABOUT_TO_SUBMIT), this::configureScenarioForTrialArrangementsNotifyOtherParty)
    ...
    private CallbackResponse configureScenarioForTrialArrangementsNotifyOtherParty(CallbackParams callbackParams) {
        CaseData caseData = callbackParams.getCaseData();
        String authToken = callbackParams.getParams().get(BEARER_TOKEN).toString();

        dashboardApiClient.recordScenario(caseData.getCcdCaseReference().toString(),
                                          SCENARIO_AAA6_CP_TRIAL_ARRANGEMENTS_NOTIFY_OTHER_PARTY_CLAIMANT.getScenario(), authToken,
