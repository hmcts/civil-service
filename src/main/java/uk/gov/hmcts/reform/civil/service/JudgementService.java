...
public class JudgementService {
    ...
    public BigDecimal ccjJudgmentClaimAmount(CaseData caseData) {
        ...
        if (isLrFullAdmitRepaymentPlan(caseData) || isLrFullAdmitPayImmediately(caseData)) {
    ...
    public BigDecimal ccjJudgmentFixedCost(CaseData caseData) {
        if ((isLrFullAdmitRepaymentPlan(caseData)
            ...
            BigDecimal claimIssueFixedCost = MonetaryConversions.penniesToPounds(
    ...
    public BigDecimal ccjJudgmentFinalTotal(CaseData caseData) {
        return ccjJudgementSubTotal(caseData).subtract(ccjJudgmentPaidAmount(caseData));
    }
    ...
    private String ccjJudgmentStatement(CaseData caseData) {
        if (caseData.isLRvLipOneVOne() && featureToggleService.isPinInPostEnabled()) {
            ...
            if (featureToggleService.isJudgmentOnlineLive() && hasPaymentOption) {
            ...
            return String.format(
                    isLrFullAdmitRepaymentPlan(caseData) || isLrPayImmediatelyPlan(caseData)
                            ? JUDGEMENT_ORDER_V2
                            : JUDGEMENT_ORDER,
                    ccjJudgementSubTotal(caseData).toString());
        }
    }
    ...
    public boolean isLrFullAdmitRepaymentPlan(CaseData caseData) {
        return isLRAdmissionRepaymentPlan(caseData) && caseData.isFullAdmitClaimSpec();
    }

    public boolean isLRPartAdmitRepaymentPlan(CaseData caseData) {
        return isLRAdmissionRepaymentPlan(caseData) && caseData.isPartAdmitClaimSpec();
    }
    ...
    public boolean isLrPayImmediatelyPlan(CaseData caseData) {
        return isLrvLrOneVOneBulkAdmissionsEnabled(caseData) && caseData.isPayImmediately();
    }

    public boolean isLrFullAdmitPayImmediately(CaseData caseData) {
        return isLrPayImmediatelyPlan(caseData) && caseData.isFullAdmitClaimSpec();
    }
