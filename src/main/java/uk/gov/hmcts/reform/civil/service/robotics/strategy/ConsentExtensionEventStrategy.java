...
public class ConsentExtensionEventStrategy implements EventHistoryStrategy {
    ...
    public boolean supports(CaseData caseData) {
        ...
        boolean hasExtensionData =
                defendant1ExtensionExists.test(caseData) || defendant2ExtensionExists.test(caseData);
        boolean hasAgreedDeadline =
    ...
    public void contribute(EventHistory eventHistory, CaseData caseData, String authToken) {
        ...
        log.info(
                "Building consent extension robotics events for caseId {}", caseData.getCcdCaseReference());
        ...
        if (defendant1ExtensionExists.test(caseData)) {
            events.add(
                    buildConsentExtensionEvent(eventHistory, PartyUtils.respondent1Data(caseData), scenario));
        }

        if (defendant2ExtensionExists.test(caseData)) {
            events.add(
                    buildConsentExtensionEvent(eventHistory, PartyUtils.respondent2Data(caseData), scenario));
        }

        List<Event> updatedConsentExtensionFilingDefenceEvents1 =
                eventHistory.getConsentExtensionFilingDefence() == null
                        ? new ArrayList<>()
                        : new ArrayList<>(eventHistory.getConsentExtensionFilingDefence());
        updatedConsentExtensionFilingDefenceEvents1.addAll(events);
        eventHistory.setConsentExtensionFilingDefence(updatedConsentExtensionFilingDefenceEvents1);
    }

    private Event buildConsentExtensionEvent(
            EventHistory builder, PartyData party, MultiPartyScenario scenario) {
        return createEvent(
                sequenceGenerator.nextSequence(builder),
                EventType.CONSENT_EXTENSION_FILING_DEFENCE.getCode(),
                party.getTimeExtensionDate(),
                resolveLitigiousPartyId(party.getRole()),
    ...
    private String getExtensionEventText(MultiPartyScenario scenario, PartyData party) {
        ...
        return switch (scenario) {
            case ONE_V_TWO_ONE_LEGAL_REP ->
                format("Defendant(s) have agreed extension: %s", extensionDate);
                ...
                .anyMatch(
                        name ->
                                FlowState.Main.NOTIFICATION_ACKNOWLEDGED_TIME_EXTENSION.fullName().equals(name)
                                        || FlowState.Main.CLAIM_DETAILS_NOTIFIED_TIME_EXTENSION
                                                .fullName()
                                                .equals(name));
    }
}
