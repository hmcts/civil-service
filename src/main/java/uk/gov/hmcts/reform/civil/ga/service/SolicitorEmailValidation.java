...
public class SolicitorEmailValidation {
    ...
    private GASolicitorDetailsGAspec updateSolDetails(
            String updateEmail, GASolicitorDetailsGAspec generalAppSolicitor) {
        return generalAppSolicitor.copy().setEmail(updateEmail);
    }

    private boolean checkIfOrgIdAndEmailAreSame(
            String organisationID,
            String civilSolEmail,
    ...
    private GASolicitorDetailsGAspec checkIfOrgIDMatch(
            GASolicitorDetailsGAspec generalAppSolicitor,
    ...
    public GeneralApplicationCaseData validateSolicitorEmail(
        ...
        if (!gaForLipService.isGaForLip(gaCaseData)) {
            caseDataBuilder.generalAppApplnSolicitor(
                    checkIfOrgIDMatch(
            ...
            gaCaseData
                    ...
                    .forEach(
                            rs ->
                                    generalAppRespondentSolicitors.add(
                                            element(
                                                    checkIfOrgIDMatch(
                                                            rs.getValue(),
                                                            civilCaseData,
    ...
    private void checkApplicantLip(
            GeneralApplicationCaseData gaCaseData,
            GeneralApplicationCaseData caseDataBuilder,
            String userEmail) {
        if (userEmail != null
            ...
            caseDataBuilder.generalAppApplnSolicitor(
                    updateSolDetails(userEmail, gaCaseData.getGeneralAppApplnSolicitor()));
        }
    }

    private void checkRespondentsLip(
            GeneralApplicationCaseData gaCaseData,
            GeneralApplicationCaseData caseDataBuilder,
            String userEmail) {
        List<Element<GASolicitorDetailsGAspec>> generalAppRespondentSolicitors = newArrayList();
