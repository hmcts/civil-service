...
public class QueryDocumentHandler extends CallbackHandler {
    ...
    private CallbackResponse handleGeneratingHearingDocument(CallbackParams callbackParams) {
        ...
        String parentQueryId = nonNull(latestCaseMessage.getParentId()) ? latestCaseMessage.getParentId() : latestCaseMessage.getId();
        ...
        updateQueryDocuments(messageThread.get(0).getValue().getCreatedOn(), caseDocument, userAuthToken, builder);
    ...
    private void updateQueryDocuments(LocalDateTime timeQueryRaised, CaseDocument newQueryDocument, String userAuth, CaseData.CaseDataBuilder builder) {
        CaseData caseData = builder.build();
        Element<CaseDocument> existingQueryDocument = caseData.getQueryDocuments().stream().filter(doc -> doc.getValue().getCreatedDatetime()
                .equals(timeQueryRaised)).findFirst()
            .orElse(null);

        if (nonNull(existingQueryDocument)) {
            caseData.getQueryDocuments().remove(existingQueryDocument);
            documentManagementService.updateDocumentTimeToLive(existingQueryDocument.getValue().getDocumentLink().getDocumentUrl(), userAuth, LocalDateTime.now());
        }
