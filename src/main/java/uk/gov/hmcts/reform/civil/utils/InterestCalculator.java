...
public class InterestCalculator {
        ...
        if(caseData.getClaimInterest() == YesOrNo.YES) {
            if(caseData.getInterestClaimOptions().name() == "SAME_RATE_INTEREST") {
                ...
                if(caseData.getSameRateInterestSelection().getSameRateInterestType().name() == "SAME_RATE_INTEREST_DIFFERENT_RATE") {
                    return calculateInterestAmount(caseData, caseData.getSameRateInterestSelection().getADifferentRate());
                }
            }
    ...
    private BigDecimal calculateInterestAmount(CaseData caseData, BigDecimal interestRate) {
        ...
        if(caseData.getInterestClaimFrom().name() == "FROM_A_SPECIFIC_DATE") {
            ...
            if(caseData.getInterestClaimUntil().name() == "UNTIL_CLAIM_SUBMIT_DATE" || caseData.getInterestClaimUntil().name() == "UNTIL_SETTLED_OR_JUDGEMENT_MADE" ) {
                ...
                BigDecimal numberOfDays = new BigDecimal(calculateDaysBetweenDates(caseData.getInterestFromSpecificDate()));
                BigDecimal interestForAYear = caseData.getTotalClaimAmount().multiply(interestRate.divide(new BigDecimal(100)));
    ...
    public long calculateDaysBetweenDates(LocalDate specificDate) {
        ...
        Date today = new Date();
        long diff = today.getTime() - Date.from(specificDate.atStartOfDay(ZoneId.systemDefault()).toInstant()).getTime();
