...
public class CaseEventTaskHandler extends BaseExternalTaskHandler {
    ...
    private String getSummary(String eventId, String state, CaseData caseData) {
        if (isProceedsInHeritageEvent(eventId)) {
            FlowState.Main flowState = (FlowState.Main) FlowState.fromFullName(state);
            return getProceedsInHeritageSummary(flowState, caseData, state);
        }
    ...
    private String getProceedsInHeritageSummary(FlowState.Main flowState, CaseData caseData, String state) {
        String summary = getFirstAvailableSummary(
            () -> getSpecialProceedsSummary(flowState),
            () -> getAdmissionOrCounterClaimSummary(flowState, caseData, state),
    ...
    private String getAdmissionOrCounterClaimSummary(FlowState.Main flowState, CaseData caseData, String state) {
        return switch (flowState) {
            case FULL_ADMISSION, PART_ADMISSION -> getFullOrPartAdmission(caseData, state);
    ...
    private String getClaimantProceedSummary(FlowState.Main flowState) {
        return switch (flowState) {
            case FULL_DEFENCE_PROCEED, FULL_ADMIT_PROCEED, FULL_ADMIT_PAY_IMMEDIATELY, PART_ADMIT_PAY_IMMEDIATELY,
                PART_ADMIT_PROCEED -> textFormatter.claimantsProceed();
    ...
    private String getSpecDefendantNocSummary(FlowState.Main flowState) {
        return flowState == FlowState.Main.SPEC_DEFENDANT_NOC
            ? textFormatter.withRpaPrefix("Notice of Change filed.")
            : null;
    }
    ...
    private String getFlowStateDescription(FlowState.Main flowState, CaseData caseData) {
        ...
        return switch (flowState) {
            case PENDING_CLAIM_ISSUED_UNREPRESENTED_DEFENDANT_ONE_V_ONE_SPEC,
                PENDING_CLAIM_ISSUED_UNREPRESENTED_DEFENDANT ->
                format("Unrepresented defendant: %s",
                       StringUtils.join(getDefendantNames(UNREPRESENTED, caseData), and));
            case PENDING_CLAIM_ISSUED_UNREGISTERED_DEFENDANT ->
                format("Unregistered defendant solicitor firm: %s",
            ...
            case FULL_DEFENCE_PROCEED ->
                !SPEC_CLAIM.equals(caseData.getCaseAccessCategory())
                    ? getDescriptionFullDefenceProceed(caseData)
                    : null;
            default -> null;
        };
    }
