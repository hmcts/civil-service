...
public class HwfNotificationService implements NotificationDataGA {
    ...
    public void sendNotification(GeneralApplicationCaseData caseData) throws NotificationException {
        sendNotification(caseData, null);
    }

    public void sendNotification(GeneralApplicationCaseData caseData, CaseEvent event) throws NotificationException {
    ...
    @Override
    public Map<String, String> addProperties(GeneralApplicationCaseData caseData, GeneralApplicationCaseData mainCaseDate) {
        return getCommonProperties(caseData, mainCaseDate);
    }

    private Map<String, String> addAllProperties(GeneralApplicationCaseData caseData, GeneralApplicationCaseData mainCaseData, CaseEvent event) {
    ...
    private Map<String, String> getCommonProperties(GeneralApplicationCaseData caseData, GeneralApplicationCaseData mainCaseData) {
    ...
    private Map<String, String> getPartialRemissionProperties(GeneralApplicationCaseData caseData) {
        BigDecimal remission;
        BigDecimal outstanding;
        if (caseData.isHWFTypeApplication()) {
            remission = MonetaryConversions
                    .penniesToPounds(HwFFeeTypeUtil.getGaRemissionAmount(caseData));
            outstanding =  MonetaryConversions
                .penniesToPounds(caseData.getGaHwfDetails().getOutstandingFee());
        } else {
            remission = MonetaryConversions
                    .penniesToPounds(HwFFeeTypeUtil.getAdditionalRemissionAmount(caseData));
            outstanding = MonetaryConversions
    ...
    private Map<String, String> getFurtherProperties(GeneralApplicationCaseData caseData, CaseEvent event) {
        return switch (event) {
    ...
    private Map<String, String> getNoRemissionProperties(GeneralApplicationCaseData caseData) {
        String remission;
        BigDecimal outstanding;
        if (caseData.isHWFTypeApplication()) {
    ...
    private String getHwFNoRemissionReasonWelsh(GeneralApplicationCaseData caseData) {
        if (caseData.isHWFTypeApplication()) {
    ...
    private CaseEvent getEvent(GeneralApplicationCaseData caseData) {
    ...
    private Map<String, String> getMoreInformationProperties(GeneralApplicationCaseData caseData) {
        ...
        return Map.of(
            ...
            HWF_MORE_INFO_DATE_IN_WELSH, formatDateInWelsh(moreInformation.getHwFMoreInfoDocumentDate(), false),
    ...
    private Map<String, String> getPaymentOutcomeProperties(GeneralApplicationCaseData caseData) {
