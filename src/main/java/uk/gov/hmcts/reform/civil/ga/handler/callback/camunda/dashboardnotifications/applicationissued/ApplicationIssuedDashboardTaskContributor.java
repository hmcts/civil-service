package uk.gov.hmcts.reform.civil.ga.handler.callback.camunda.dashboardnotifications.applicationissued;

import org.springframework.stereotype.Component;
import uk.gov.hmcts.reform.civil.handler.callback.camunda.dashboardnotifications.DashboardCaseType;
import uk.gov.hmcts.reform.civil.handler.callback.camunda.dashboardnotifications.DashboardTaskContributor;
import uk.gov.hmcts.reform.civil.handler.callback.camunda.dashboardnotifications.DashboardTaskIds;

@Component
public class ApplicationIssuedDashboardTaskContributor extends DashboardTaskContributor {

    public ApplicationIssuedDashboardTaskContributor(ApplicationIssuedApplicantDashboardTask applicantTask,
                                                     ApplicationIssuedRespondentDashboardTask respondentTask) {
        super(
            DashboardCaseType.GENERAL_APPLICATION,
            DashboardTaskIds.GA_APPLICATION_ISSUED,
            applicantTask,
            respondentTask
        );
    }
}
