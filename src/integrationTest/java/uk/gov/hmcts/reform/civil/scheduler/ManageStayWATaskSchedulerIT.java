...
import uk.gov.hmcts.reform.civil.scheduler.managestaywatask.ManageStayWATaskScheduledTask;
...
public class ManageStayWATaskSchedulerIT {
    ...
    @BeforeEach
    void setUp() {
        reset(telemetryService, featureToggleService, manageStayWATaskScheduledTask);
        coreCaseDataApiMockHelper.resetMocks();
        coreCaseDataApiMockHelper.setupIdamClient();
        when(featureToggleService.isSpringSchedulerEnabled(SCHEDULER_NAME)).thenReturn(true);
        when(manageStayWATaskScheduledTask.maxCasesPerRun()).thenReturn(Long.MAX_VALUE);
        when(manageStayWATaskScheduledTask.getItemId(any(CaseDetails.class))).thenAnswer(invocation ->
            invocation.<CaseDetails>getArgument(0).getId());
    }

    @Test
    void shouldExecuteManageStayWATaskScheduler() {
        ...
        verify(manageStayWATaskScheduledTask).accept(searchCase);
        verify(telemetryService).trackEvent(eq("ManageStayWATaskJobStarted"), anyMap());
        verify(telemetryService).trackEvent(eq("ManageStayWATaskCaseProcessed"), anyMap());
        verify(telemetryService).trackEvent(eq("ManageStayWATaskJobCompleted"), anyMap());
    }
}
