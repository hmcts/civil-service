    ...
    void should_create_claimant_requested_judgment_by_admission() throws Exception {
        ...
        doGet(BEARER_TOKEN, GET_NOTIFICATIONS_URL, caseId, "CLAIMANT").andExpect(status().isOk()).andExpectAll(
            ...
            jsonPath("$[0].descriptionEn").value(
                "<p class=\"govuk-body\">You accepted the <a href=\"{VIEW_CCJ_REPAYMENT_PLAN_CLAIMANT}\" class=\"govuk-link\">repayment plan</a>.<br>When we've processed the request, we'll post a copy of the judgment to you.</p>"),
