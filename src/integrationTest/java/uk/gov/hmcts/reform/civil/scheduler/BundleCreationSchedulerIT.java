...
public class BundleCreationSchedulerIT {
    ...
    @BeforeEach
    void setUp() {
        reset(telemetryService, featureToggleService, bundleCreationScheduledTask);
        coreCaseDataApiMockHelper.resetMocks();
        coreCaseDataApiMockHelper.setupIdamClient();
        when(featureToggleService.isSpringSchedulerEnabled(SCHEDULER_NAME)).thenReturn(true);
        when(bundleCreationScheduledTask.maxCasesPerRun()).thenReturn(Long.MAX_VALUE);
        when(bundleCreationScheduledTask.getItemId(any(CaseDetails.class))).thenAnswer(invocation ->
