...
curl --silent -v --show-error \
  ...
  -X POST "${IDAM_API_URL}/o/token?grant_type=password&redirect_uri=${REDIRECT_URI}&client_id=${CLIENT_ID}&client_secret=${CLIENT_SECRET}&username=${USERNAME}&password=${PASSWORD}&scope=${SCOPE}" -d "" | docker run --rm --interactive ghcr.io/jqlang/jq:latest -r .access_token